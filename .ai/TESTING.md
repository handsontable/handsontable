...
|---|---|---|---|---|
| Core unit | Jest (jsdom, `jest-jasmine2` runner) | `*.unit.js` | `pnpm --filter handsontable run test:unit` | [`handsontable/.ai/TESTING.md`](../handsontable/.ai/TESTING.md) |
| Core E2E | Jasmine + Puppeteer (headless Chrome) | `*.spec.js` | `pnpm --filter handsontable run test:e2e` | [`handsontable/.ai/TESTING.md`](../handsontable/.ai/TESTING.md) |
| Type tests | `tsc` only | `*.types.ts` | `pnpm --filter handsontable run test:types` | [`handsontable/.ai/TESTING.md`](../handsontable/.ai/TESTING.md) |
| Walkontable | Separate runner (`SpecRunner.html`) | — | `pnpm --filter handsontable run test:walkontable` | `handsontable/src/3rdparty/walkontable/AGENTS.md`; skills `walkontable-testing`, `walkontable-dev` |
| React wrapper | Jest + `@testing-library/react` | — | `pnpm --filter @handsontable/react-wrapper run test` | `wrappers/react-wrapper/AGENTS.md` |
| Vue 3 wrapper | Jest + `@vue/test-utils` | — | `pnpm --filter @handsontable/vue3 run test` | `wrappers/vue3/AGENTS.md` |
| Angular wrapper | `jest-preset-angular` | — | `pnpm --filter @handsontable/angular-wrapper run test` | `wrappers/angular-wrapper/AGENTS.md` |
| Visual regression | Playwright + Argos | — | `pnpm --filter handsontable-visual-tests run test` | `visual-tests/AGENTS.md`; skills `visual-testing`, `creating-visual-test-examples` |
...
Run commands from the monorepo root with `pnpm --filter <package> run <task>`,
