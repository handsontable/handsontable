...
**Important:** Type declarations are auto-generated by `tsc --emitDeclarationOnly` (`build:types` task in `scripts/tasks.json`, uses `tsconfig.build-types.json`) and output to `handsontable/tmp/*.d.ts`. Do not hand-edit them. The `handsontable/types/` directory has been deleted — do not recreate it. `tsconfig.json` uses `strict: false`, `noImplicitAny: true`. Walkontable (`src/3rdparty/walkontable/`) is also TypeScript, with its own separate build/test pipeline. There is no `src/common.ts`; central shared types live in `src/core/settings.ts` (`GridSettings`, `Events`, `HookKey`) and `src/core/types.ts` (`HotInstance`).
