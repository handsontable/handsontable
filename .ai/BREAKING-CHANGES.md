...
So a new export added to one of those modules lands on the global object automatically, and that is **additive, not breaking**: it needs no changelog line, no docs page and no deprecation cycle. `runEveryStep` (helpers/function, PR #13503) is the reference case. Use the `_` prefix only when you want the runtime copy skipped — `_injectProductInfo` and `_getLicenseState` in `helpers/mixed.ts` do — and know that the type still lists it, so the prefix hides it from the object, not from TypeScript.
