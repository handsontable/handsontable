...
- **Avoid spread operator with large arrays**: When working with potentially large arrays (10k+ elements), avoid using the spread operator to push elements (e.g., `arr.push(...largeArray)`). This creates excessive function arguments and can cause stack overflow errors. Use `for...of` loops instead.
