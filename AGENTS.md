...
- `manual-qa.yml` is a tickbox-armed manual gate inside the Tests DAG: ticking "MANUAL QA NEEDED" in the PR description makes its `sign-off` job wait on the **`manual-qa` environment** approval (designated reviewers; self-review blocked), which holds CI Gate through its `needs` edge — unticked PRs skip the module (skipped, never a misleading green, no runner spent). The Checks scope router reads the box live, and `pr-manual-qa-label.yml`'s `arm` job enforces the invariant *ticked box ⇒ the head commit's Tests run contains the gate*: it re-runs a Tests run that went green without the module. That check is **state-based and idempotent**, and it runs on PR events **and** on `workflow_run` completion of Tests — the completion trigger is what covers a box ticked while the pipeline is still running, which a PR event alone cannot fix (an in-progress run cannot be re-run). It never auto-disarms: dropping a gate needs a human "Re-run all jobs" (or a push). The sign-off job additionally **fails closed** — it asserts an approval is recorded for the run via the approvals API, so an unprotected or drifted `manual-qa` environment turns CI Gate red instead of silently waving PRs through. The template line's wording is the router's regex contract — don't reword either side. The environment must exist **with required reviewers** before the first ticked PR — a job referencing a missing environment auto-creates it unprotected, and the gate would wave everything through. `pr-manual-qa-label.yml` is the only label writer in the repo (every other label — `Breaking change`, `QA needed`, `run-docs-visual` — is applied by hand, and GitHub's `actions/labeler` cannot read a PR body): it applies the red `Requires Manual QA` marker and does the arming in one job. **Apply-only** — unticking removes nothing, by request. It is standalone because it needs `issues: write` + `pull-requests: write` (a PR label write is billed to both) plus `actions: write` for the re-run, and a nested module job must not request those. Its `workflow_run` half only ever runs from the **default branch's** copy of the file, so changes to it cannot be exercised on the PR that makes them.
