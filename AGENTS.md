...
- **`test-health.yml` is the cross-run flake ledger, and it is `workflow_run`-chained on purpose.** It runs after every completed `Tests` or `Develop` run; when that run was red it downloads the run's `playwright-report-*` (the Playwright JSON report) and `puppeteer-failed-specs-*` artifacts and appends every flaky or failed test to `test-health/ledger.json` on `gh-pages`, published at <https://handsontable.github.io/handsontable/test-health/> — per test: 7- and 30-day counts, distinct runs, legs hit, last sighting, and a "needs ticket" flag at 2+ distinct runs in 30 days, the flakiness playbook's line for a fix or migration ticket. Two consequences of the trigger. It runs in the base repository's context with a write token whatever started the run, so fork and Dependabot flakes are recorded with **no** fork guard (it is deliberately absent from the guarded-site list above). And it always runs the copy on the **default** branch, so a change to it is exercised only after it lands on `develop`: use its `workflow_dispatch` with a run id to test it there, and to backfill a run whose artifacts (3-day retention) still exist. It has no `concurrency` group by design — GitHub keeps one pending run per group and cancels the rest, which would drop collections when several runs finish together — and instead re-reads `gh-pages` and re-runs the idempotent collector before every push attempt. The pure half is `.github/scripts/lib/test-health.mjs`; `.github/scripts/__tests__/test-health.test.mjs` also pins the JSON report path across `tests/playwright.config.ts`, `e2e.yml` and the collector, so moving it means changing all three. Hand-recorded observations (a flake seen before the collector existed, or only locally) go in `.github/test-health/seed.json` and are merged on every publish.
