...
- **`postcss` is pinned below 8.5.24 because 8.5.24 corrupts the minified CSS.** From 8.5.24 on, the `css-loader` -> `CssExtractRspackPlugin` -> `LightningCssMinimizerRspackPlugin` chain emits a UTF-8 BOM at the head of the minified CSS body, immediately after the license banner. Nothing rejects it: `handsontable.min.css` still parses, the *unminified* build is unaffected (it keeps its `@charset "UTF-8"`, which sass emits because the themes contain a non-ASCII glyph), and the only visible difference is three bytes of file size. But `css-to-js-export-plugin.js` then reads that file with `fs.readFileSync(path, 'utf8')` — which does **not** strip a BOM — and `JSON.stringify`s it into `src/styles/handsontableStyles.{js,ts}`, so the bundles that inline the runtime theme string carry a `\uFEFF` before the first selector. That kills the first rule, `.ht-root-wrapper`, and the grid's root wrapper loses its layout: cells stop receiving pointer events and popovers mis-anchor. Only the `full`/`.min` E2E legs see it, because they are the only ones that load the inlined string, so the failure reads as three unrelated `dblclick` timeouts on one bundle axis rather than as a CSS regression. The root override is therefore `postcss: '>=8.5.18 <8.5.24'` — `>=8.5.18` is what GHSA-r28c-9q8g-f849 requires, `<8.5.24` is what the BOM requires. Widen the upper bound only after checking that `handsontable/styles/handsontable.min.css` still has no `EF BB BF` and that `dist/handsontable.full.min.js` contains exactly one `\uFEFF` (HyperFormula's own whitespace table). The `examples/next/*/react-wrapper` npm overrides deliberately stay at `^8.5.18`: those trees build through Create React App's postcss chain, never through lightningcss, and are unaffected.
- **When a lockfile experiment gives a nonsensical answer, suspect the working tree before the theory.** `pnpm install --frozen-lockfile` does not always relink a package whose version changed, so switching branches back and forth leaves `handsontable/node_modules` pointing at the *other* branch's copy while `pnpm-lock.yaml` reads correctly — which made `develop` reproduce a bug it does not have. Two further traps compound it: removing a `pnpm.overrides` entry does **not** roll the version back (pnpm keeps any lock entry that still satisfies the now-unconstrained range, so a bisect by deletion silently tests nothing), and `pnpm install --lockfile-only` is sticky in the same way. Bisect dependency behavior by **pinning each candidate to the exact old version**, in a throwaway `git worktree` with its own `pnpm install`, never by deleting overrides in the main checkout.
- **Dependabot remediation runs through two different override mechanisms, and the npm one silently no-ops.** Nearly every alert lands on a *transitive* dependency, so bumping a declaration is rarely the lever. The monorepo tree is governed by `pnpm.overrides` in the root `package.json` — scope each entry as narrowly as the advisory (a bare name where one major is installed, `name@<major>` where several are, `parent>child` where only one consumer is affected), then `pnpm install --lockfile-only`. The `examples/next/<category>/<framework>/` trees are **separate npm workspaces with their own committed `package-lock.json`**, which inherit nothing from the root; they need an `overrides` block in the framework-level `package.json`. The trap: `npm install` applies a *new* `overrides` entry only when it re-resolves from scratch. With a valid `package-lock.json` present, or with a `node_modules/` on disk (including a per-example `<example>/node_modules/`, which is easy to miss), npm prints `up to date` and ignores the block, so the lock looks regenerated while the vulnerable version stays pinned. Generate the lock in a throwaway directory holding only the framework `package.json` and its examples' `package.json` files, then copy the result back. **That full re-resolve is the expensive option, so weigh it against the bullet above**: it floats the whole tree (61-211 packages per lock in DEV-2772, `handsontable` 18.0.0 -> 18.1.0 among them, which then broke the react demos' pinned `typescript@4.2.4` on the grid's own inline `type` imports), and those trees ship in no bundle, which is exactly why their audit is report-only. A **direct** dependency bump in a leaf manifest — the usual security fix there, e.g. `vite` — needs none of this: a plain `npm install` with the lockfile left in place picks it up and re-resolves nothing else. Reach for the throwaway re-resolve only when the fix genuinely has to travel through `overrides`, and say in the PR why the float was worth it. After any lockfile work, confirm the compile floor did not move: `core-js`, `hyperformula`, `typescript` and `esbuild` must be byte-identical, and `browserslist`/`caniuse-lite` may only move when an advisory demands it (`BROWSERS_LIST` is pinned integers, so caniuse data cannot shift the target — see the ES-floor entry above).
- **Four high-severity Dependabot alerts have no fix and must not be chased.** `extract-zip@2.0.1` (via `@puppeteer/browsers`) and `image-size@0.5.5` (an optional dependency of `less`) have no patched release at all. `xlsx@0.18.5` is the last SheetJS build ever published to npm — patched versions exist only on the SheetJS CDN, and it is kept here purely so the TypeScript docs examples type-check, while the browser examples load it from a CDN `<script>`. `svgo@1.3.2` in the two `react-wrapper` example trees is capped by `@svgr/plugin-svgo@5` under `react-scripts@5.0.1`; forcing svgo 2 breaks the SVG loader, so it clears only when those Create React App examples migrate to Vite. The `@angular/*` 19.2.x alerts are the same shape: Angular has not backported them, so they clear only when the wrapper's support matrix moves.
