...
- **A linked worktree under `.claude/worktrees/` is not a working checkout until you bootstrap it.** `git worktree` materializes tracked files only, so the per-package `node_modules`, `.claude/settings.local.json`, the code-review graph, and the Claude project memory are all absent. Run `node scripts/claude/setup-worktree.mjs` once per worktree. Two traps to know before you debug anything there: a **symlinked root `node_modules`** resolves root binaries (so lint passes) while every package-local `.bin` is missing, and package scripts then die mid-build with a bare `env-cmd: command not found` (exit 127); and the Playwright suites use fixed ports with `reuseExistingServer`, so a second checkout **silently tests the first one's build** — check `lsof -i :8123` before believing a result, or set `HOT_TEST_PORT` (functional suite only; the visual suite's port is owned by `visual-tests/src/config.mjs` and two hardcoded `app.listen(8082)` demo servers, so it cannot be moved from the Playwright config). None of this affects a normal clone: the bootstrap and its SessionStart check are inert outside a worktree. Full list: **`.ai/WORKTREES.md`**.
