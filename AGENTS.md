...
- **Fork pull requests get a read-only `GITHUB_TOKEN` and no repository secrets.** Any step that posts a PR comment, pushes to `gh-pages`, or reads a secret must be guarded, or it 403s and takes `CI Gate` (`test.yml`'s single required check, which passes only on `success|skipped`) down with it, making every external contribution unmergeable. The canonical guard, used at every such site (`.github/actions/performance-run/action.yml`, `integration.yml`'s `preview-packages`, `code-quality.yml`'s `sonarcloud`/`fossa`, `visual.yml`'s Argos upload, `docs.yml`'s `preview`):
  ```yaml
  if: github.event_name != 'pull_request'
    || github.event.pull_request.head.repo.full_name == github.repository
  ```
  The `github.event_name != 'pull_request'` half is **mandatory**, not defensive padding: on the RC path `publish.yml` calls `test.yml`, `github.event.pull_request` is null there, and a bare `!= github.repository` would read as "fork" and silently disable the step on every release run. Prefer **step-level** guards where the job still does useful work (perf still computes numbers, `preview-packages` still publishes to pkg.pr.new, the visual matrix still renders screenshots); job-level only where the job is entirely secret-dependent. Fork PRs see the withheld content via `$GITHUB_STEP_SUMMARY`, which needs no token. Never reach for `pull_request_target` to fix this: it would run fork-controlled code that builds and publishes preview packages with a write token.
