...
**The core package (`handsontable/`) is TypeScript.** Source files in `handsontable/src/` are `.ts`. Type definitions are auto-generated by `tsc --emitDeclarationOnly` (`build:types` task in `scripts/tasks.json`) into `handsontable/tmp/` — do not hand-edit them. The `handsontable/types/` directory has been deleted; do not recreate it.
...
- TypeScript declarations are auto-generated into `handsontable/tmp/` — regenerate them with `npm run build:types` (runs `tsc -p tsconfig.build-types.json`) after changing source files. The `handsontable/types/` directory no longer exists; do not recreate it or hand-edit `tmp/`.
