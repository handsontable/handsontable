...
**Production and staging are served by Cloudflare Pages, not Netlify.** Netlify is retained only for PR previews during the migration to Cloudflare. Do not assume Netlify is production - see `README-DEPLOYMENT.md` for the current platform table.

When you rename a page's `permalink` (or remove a page), add redirect rules in **both** of these hand-maintained files - they must stay in sync:

- `cloudflare/_worker.js` - the **production** authority. Cloudflare Pages ignores `_redirects` when a `_worker.js` is present, so the worker re-implements every redirect rule. Add old-slug-to-new-slug exact-path entries to its `crossFramework` map (matched before the static-asset fallthrough), one per framework prefix.
- `netlify/_redirects` - used for Netlify PR previews. Add the equivalent per-prefix lines. Never point a `:major.:minor` versioned wildcard at the new slug; that wildcard matches frozen older versions where the page still lives at the old slug.
