...
`docs.handsontable.com` is a legacy custom domain on the `handsontable-docs` project. It pre-dates the move to `handsontable.com/docs` and is kept working for old bookmarks and inbound links, but every `GET`/`HEAD` on it answers with a `301` to the matching page on `handsontable.com`. The redirect is implemented in `docs/cloudflare/_worker.js` (rule 12a, see `LEGACY_DOCS_HOSTS`) rather than in the Cloudflare dashboard, so it stays in version control with the rest of the routing rules.

Two details are load-bearing. The rule sits **above** the cookie-reading rules 13-17 on purpose: those answer with a `302` because their destination depends on the `docs_fw` cookie, and a `302` is not a canonicalisation signal, so a legacy URL answered by one would never leave the search index. Placing the cut there also moves the framework decision to the host that actually holds the cookie, since cookies are host-scoped. And do not add a `*.pages.dev` apex to `LEGACY_DOCS_HOSTS` - the staging apex and the per-PR previews are served from the same worker, so redirecting them would break documentation review.
