...
const wrap = (code, lang) => {
  ...
  return `<pre v-pre class="language-${lang}"><code>${code}</code></pre><button @click="$parent.$parent.copyCode($event)" class="copycode" aria-label="Copy to clipboard"><i class="ico i-copy no-pointer"></i><i class="ico i-checks no-pointer"></i></button><a class="report" href="https://github.com/handsontable/handsontable/issues/new/choose" aria-label="Report an issue" target="_blank"><i class="ico i-report"></i></a>`;
};
