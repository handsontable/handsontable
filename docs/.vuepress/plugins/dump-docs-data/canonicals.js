...
logger.info(`Using "${BASE_DOCS_URL}/docs/data/common.json" URL for fetching and building Docs data ` +
...
async function generateCommonCanonicalURLs(currentCanonicals) {
  ...
  const response = await fetch(`${BASE_DOCS_URL}/docs/data/common.json`, {
  ...
  for (let vIndex = 0; vIndex < allDocsVersions.length; vIndex++) {
    ...
    if (currentCanonicals.v === docsVersion) {
      ...
      const canonicalsResponse = await fetch(`${BASE_DOCS_URL}/docs/${docsVersion}/data/canonicals-raw.json`, {
        headers: fetchCommonHeaders
      });
