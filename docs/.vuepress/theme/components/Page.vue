...
export default {
  ...
  methods: {
    ...
    submitCodesandbox(submitEvent) {
      ...
      const preset = submitEvent.target.elements.preset.value;
      const css = submitEvent.target.elements.css.value;

      const formatVersion = version =>
        (/^\d+\.\d+$/.test(version) ? version : 'latest');

      const hyperformula = js.includes('hyperformula')
        ? { hyperformula: 'latest' }
        : {};
      ...
      const packageContent = isReact
        ? {
          ...
          dependencies: {
            ...
            // eslint-disable-next-line
            '@handsontable/react': formatVersion(DOCS_VERSION),
  ...
  document.querySelector("#${id}")
      ...
      fetch('https://codesandbox.io/api/v1/sandboxes/define?json=1', {
        ...
        .then((data) => {
          ...
          if (data.sandbox_id) {
            window.open(
              `https://codesandbox.io/p/sandbox/${data.sandbox_id}`,
              '_blank',
              'noreferrer'
            );
          }
