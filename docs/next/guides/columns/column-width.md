...
By default, the column width adjusts to the width of the content. The minimum width is `50px`, including `1px` for borders on the sides. The column size can be passed as a `constant`, an `array`, or a `function`.
