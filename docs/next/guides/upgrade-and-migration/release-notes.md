...
- **Breaking change**: Changed how Handsontable defines its default keyboard shortcuts, to improve consistency of user experience. Now, all default keyboard shortcuts are defined explicitly, and properly documented. Implicit keyboard shortcuts no longer work (e.g. <kbd>Enter</kbd> opens a cell's editor, but <kbd>Ctrl</kbd> + <kbd>Enter</kbd> doesn't). This change doesn't affect custom keyboard shortcuts defined through the [`beforeKeyDown`](@/api/hooks.md#beforekeydown) and [`afterDocumentKeyDown`](@/api/hooks.md#afterdocumentkeydown) hooks. [#8942](https://github.com/handsontable/handsontable/issues/8942)[[Guides: Keyboard shortcuts](@/guides/accessories-and-menus/keyboard-shortcuts.md)]
