...
| `checked` | Whether the item is marked with a check mark. Can be a `boolean`, the string `'mixed'`, or a function returning either. When a function, `this` refers to the Handsontable instance. Use `'mixed'` when the state applies to only part of the selection: the item shows a dash instead of a check mark, and assistive technology announces it as partly checked. Setting this option also makes the item a checkbox for assistive technology, so its state is announced as well as shown. An item that defines its own `renderer` draws its own content, so it gets the accessible state but no mark. |
