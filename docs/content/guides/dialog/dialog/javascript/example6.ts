...
(document.getElementById('content-direction-select') as HTMLElement).addEventListener('change', (event) => {
  dialogPlugin.update({
    contentDirections: (event.target as HTMLSelectElement).value,
  });
});
