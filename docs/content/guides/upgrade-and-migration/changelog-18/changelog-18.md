...
#### Added
- Added the [`modifySinglePassLayout`](@/api/hooks.md#modifysinglepasslayout) hook, which forces the previous measure-then-render layout path for a table. [#12951](https://github.com/handsontable/handsontable/pull/12951)
- Added the [`selectionHandles`](@/api/options.md#selectionhandles) option, which shows draggable handles at each edge midpoint of a selected range for resizing the selection, and the [`moveCells`](@/api/options.md#movecells) option for moving a cell selection to a new location by dragging its border. [#13076](https://github.com/handsontable/handsontable/pull/13076)
...
- Added the `intl-datetime` cell type, with a native date-time picker, `Intl.DateTimeFormat` display via the new [`dateTimeFormat`](@/api/options.md#datetimeformat) option, and column sorting, filtering, and Excel export support. [#13151](https://github.com/handsontable/handsontable/issues/13151)
- Added the [`preserveNumericLiteral`](@/api/options.md#preservenumericliteral) option to the numeric cell type, which keeps the exact value you type in the cell editor (preserving trailing decimal zeros and large-number precision). [#13170](https://github.com/handsontable/handsontable/pull/13170)
- Added the [`colorScheme`](@/api/options.md#colorscheme) and [`density`](@/api/options.md#density) options, which set the color scheme and the spacing of the grid without declaring a theme. See [Set the color scheme or density without a theme](@/guides/styling/themes/themes.md#set-the-color-scheme-or-density-without-a-theme). [#13205](https://github.com/handsontable/handsontable/pull/13205)
- Added a public API to the [`NestedRows`](@/api/nestedRows.md) plugin for collapsing and expanding parent rows: [`collapseAll()`](@/api/nestedRows.md#collapseall), [`expandAll()`](@/api/nestedRows.md#expandall), [`collapseParent()`](@/api/nestedRows.md#collapseparent), [`expandParent()`](@/api/nestedRows.md#expandparent), [`toggleParent()`](@/api/nestedRows.md#toggleparent), [`getCollapsedParents()`](@/api/nestedRows.md#getcollapsedparents), [`isParentCollapsed()`](@/api/nestedRows.md#isparentcollapsed), [`isParent()`](@/api/nestedRows.md#isparent), [`getRowLevel()`](@/api/nestedRows.md#getrowlevel), [`getRowParent()`](@/api/nestedRows.md#getrowparent), [`countChildren()`](@/api/nestedRows.md#countchildren), [`expandToRow()`](@/api/nestedRows.md#expandtorow), and [`expandToLevel()`](@/api/nestedRows.md#expandtolevel), plus the new [`beforeRowCollapse`](@/api/hooks.md#beforerowcollapse), [`afterRowCollapse`](@/api/hooks.md#afterrowcollapse), [`beforeRowExpand`](@/api/hooks.md#beforerowexpand), and [`afterRowExpand`](@/api/hooks.md#afterrowexpand) hooks. The plugin also no longer loses its collapsed rows on an [`updateSettings()`](@/api/core.md#updatesettings) call, and no longer throws when asked for the index, parent, or nesting level of a row object held from before a [`loadData()`](@/api/core.md#loaddata) or [`updateData()`](@/api/core.md#updatedata) call. [#13206](https://github.com/handsontable/handsontable/pull/13206)
- Added the [`formulas.hyperlinks`](@/api/options.md#formulas) option, which renders cells holding a `HYPERLINK` formula as clickable links. [#13223](https://github.com/handsontable/handsontable/pull/13223)
- Added the `SanitizerContext` type, which lists the write surfaces the [`sanitizer`](@/api/options.md#sanitizer) option receives. [#13240](https://github.com/handsontable/handsontable/pull/13240)
...
#### Added
- **Breaking change**: Added wrapper layout slots and a [`layout`](@/api/options.md#layout) option to control the order of UI elements rendered around the grid (pagination, dialog). The license notification always renders last in the bottom area and is not orderable. Built-in UI now mounts into dedicated wrapper containers, which changes the root DOM structure. [#12094](https://github.com/handsontable/handsontable/issues/12094)
...
- Added [`hashRevealDelay`](@/api/options.md#hashrevealdelay) option to the password cell type, which briefly shows each typed character before masking it. [#12491](https://github.com/handsontable/handsontable/issues/12491)
