...
| <span data-option="filterValueComparator" data-levels="grid columns"></span>[`filterValueComparator`](@/api/options.md#filtervaluecomparator) | Yes | Yes | No | No | Filters | Orders a column's "Filter by value" list, which is built once per column. Set it at the grid level or inside `columns`; a `cells` or `cell` value is not meaningful for a whole-column list. |
