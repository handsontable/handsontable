...
| <span data-option="autoLink" data-levels="grid columns cells cell"></span>[`autoLink`](@/api/options.md#autolink) | Yes | Yes | Yes | Yes | AutoLink | The plugin toggle is grid level. A column or cell `false` opts out, and a column or cell object overrides `target`, `schemes`, `inline`, and `className` for those cells. |
