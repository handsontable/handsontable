...
export class AppComponent {
  ...
  readonly groups = ['Top line', 'Direct costs', 'Margin', 'Overhead', 'Margin', 'Bottom line'];
  ...
  readonly hotSettings: GridSettings = {
    ...
    afterGetRowHeaderRenderers: (renderers: Array<(renderableRowIndex: number, TH: HTMLElement) => void>) => {
      ...
      renderers.push((renderableRowIndex, TH) => {
        TH.innerText = this.groups[renderableRowIndex];
      });
      renderers.push((renderableRowIndex, TH) => {
        TH.innerText = this.lineItems[renderableRowIndex];
      });
    },
