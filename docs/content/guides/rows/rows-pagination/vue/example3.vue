...
function updateUI() {
  const plugin = (hotRef.value as any)?.hotInstance?.getPlugin('pagination');

  if (!plugin) return;
  paginationState.value = plugin.getPaginationData();
}

const isFirstPage = () => paginationState.value.currentPage === 1;
const isLastPage = () => paginationState.value.currentPage === paginationState.value.totalPages;

function handleFirstPage() {
  (hotRef.value as any)?.hotInstance?.getPlugin('pagination').firstPage();
}

function handlePrevPage() {
  (hotRef.value as any)?.hotInstance?.getPlugin('pagination').prevPage();
}

function handleNextPage() {
  (hotRef.value as any)?.hotInstance?.getPlugin('pagination').nextPage();
}

function handleLastPage() {
  (hotRef.value as any)?.hotInstance?.getPlugin('pagination').lastPage();
}

function handlePageNumberInput(event: Event) {
  const pageNumber = parseInt((event.target as HTMLInputElement).value, 10);

  if (!isNaN(pageNumber)) {
    (hotRef.value as any)?.hotInstance?.getPlugin('pagination').setPage(pageNumber);
  }
}
...
<template>
  <div id="example3">
    <div class="example-controls-container">
      <div class="controls">
        <button :disabled="isFirstPage()" @click="handleFirstPage">First page</button>
        <button :disabled="isFirstPage()" @click="handlePrevPage">Previous page</button>
        <button :disabled="isLastPage()" @click="handleNextPage">Next page</button>
        <button :disabled="isLastPage()" @click="handleLastPage">Last page</button>
        ...
        <output>{{ paginationState.firstVisibleRowIndex + 1 }} - {{ paginationState.lastVisibleRowIndex + 1 }} of {{ paginationState.totalRenderedRows }} rows</output>
      </div>
    </div>
