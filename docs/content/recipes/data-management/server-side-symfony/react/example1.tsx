...
const ExampleComponent = () => {
  ...
  const onRowsCreate = useCallback(async (payload: RowsCreatePayload): Promise<void> => {
    ...
    const data = await res.json() as SymfonyProductRow[];
    const row = data[0];
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    const pageSize: number = hot.getSettings().pagination?.pageSize ?? 10;
    const rows = hot.getSourceData() as SymfonyProductRow[];
  ...
  const beforeRowsMutation = useCallback((operation: string, payload: RowMutationPayload): false | void => {
  ...
  const filterElectronics = useCallback(() => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const filters = (hotRef.current?.hotInstance as any)?.getPlugin('filters');
    if (!filters) return;
