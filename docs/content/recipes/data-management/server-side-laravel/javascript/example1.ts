...
function applyFilters(rows: Product[], filters: Filter[] | null): Product[] {
...
// Sorts rows. sort has the shape: { prop, order } or null.
function applySort(rows: Product[], sort: Sort | null): Product[] {
  if (!sort) return rows;
...
function mockFetchProducts(params: DataProviderQueryParameters): Promise<LaravelResponse> {
  let rows = applyFilters([...PRODUCTS], params.filters as Filter[] | null);
  rows = applySort(rows, params.sort as Sort | null);
  const total = rows.length;
  const data = rows.slice((params.page - 1) * params.pageSize, params.page * params.pageSize);
  return Promise.resolve({ data, total });
}

// Simulates the Laravel controller's store() method.
function mockCreateProducts({ rowsAmount }: { rowsAmount: number }): Promise<void> {
...
function mockUpdateProducts(rows: Array<{ id: number; changes: Partial<Product> }>): Promise<void> {
...
function mockDeleteProducts(rowIds: number[]): Promise<void> {
  rowIds.forEach(id => {
...
const hot = new Handsontable(container, {
  dataProvider: {
    ...
    fetchRows: async (queryParameters, { signal }) => {
      ...
      const json = await mockFetchProducts(queryParameters);
    ...
    onRowsCreate: async (payload) => {
      ...
      //   });
      //
      await mockCreateProducts(payload as { rowsAmount: number });
    },
    ...
    onRowsUpdate: async (rows) => {
      ...
      //   });
      //
      await mockUpdateProducts(rows as Array<{ id: number; changes: Partial<Product> }>);
    },
    ...
    onRowsRemove: async (rowIds) => {
      ...
      //   });
      //
      await mockDeleteProducts(rowIds as number[]);
    },
  },
  ...
  beforeRowsMutation(operation, payload) {
    if (operation === 'remove') {
      const rowsRemove = (payload as { rowsRemove: number[] }).rowsRemove;
