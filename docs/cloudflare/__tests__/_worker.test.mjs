...
test('serves the React and Angular variants of the numbro recipe instead of bouncing them to the index', async() => {
  const worker = loadWorker();

  for (const framework of ['react', 'angular']) {
    const path = `/docs/${framework}-data-grid/recipes/cell-types/numbro/`;
    const response = await worker.fetch(request(path), env);
