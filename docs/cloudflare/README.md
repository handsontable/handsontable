...
- **Design System date (rule 18c)** - `GET /docs/api/design-system-updated.json` answers with the date the Design System Figma file was last updated. Two pages show it as a field - the design system guide and the changelog - both marking the spot with a `data-design-system-updated` element that `public/scripts/design-system-updated.js` fills in. It needs two secrets, neither of which is in the repo: **`FIGMA_TOKEN`** (a Figma token with `file_versions:read` and `file_metadata:read`, or the broader `files:read`) and **`FIGMA_FILE_KEY`** (the key of the Figma file to report on - the segment after `/design/` in that file's URL, *not* the numeric id of its Community listing).
