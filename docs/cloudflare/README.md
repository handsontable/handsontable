...
- **Design System date (rule 18c)** - `GET /docs/api/design-system-updated.json` answers with the date the Design System Figma file was last updated. Two pages show it as a field - the design system guide and the changelog - both marking the spot with a `data-design-system-updated` element that `public/scripts/design-system-updated.js` fills in. It needs two Worker secrets, neither of which is in the repo: **`FIGMA_TOKEN`** (a Figma token with `file_versions:read` and `file_metadata:read`, or the broader `files:read`) and **`FIGMA_FILE_KEY`** (the key of the Figma file to report on). Set them with `wrangler secret put <name>` or in the Cloudflare dashboard - never in code, and never in client JavaScript, where a token would be public. **Until both are set the endpoint answers `{"date": null}` and the field stays hidden**, which is the intended fallback and also the first thing to check if the date never appears. The read is server-side for a second reason too: `api.figma.com` is not in the site's `connect-src`, so a browser could not call Figma directly even with a token.
