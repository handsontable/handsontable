...
import { sessionEditsFile, stopVerdict, toRepoRelative } from '../session.mjs';

test('toRepoRelative normalizes in-repo paths and rejects those outside', () => {
  const root = '/Users/dan/hoc/handsontable';

  // Absolute in-repo path → repo-relative.
  assert.equal(
    toRepoRelative(`${root}/handsontable/src/core.ts`, root),
    'handsontable/src/core.ts',
  );
  // Already repo-relative → unchanged.
  assert.equal(toRepoRelative('tests/e2e/x.spec.ts', root), 'tests/e2e/x.spec.ts');
  // Outside the repo (e.g. a scratchpad) → null (the crash this guards against).
  assert.equal(toRepoRelative('/private/tmp/scratchpad/pr-body.md', root), null);
  assert.equal(toRepoRelative('', root), null);
});
