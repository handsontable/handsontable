...
| `getStructureVersion()` | — | A counter that changes on every row/column insert or remove. `LazyFactoryMap` re-keys stored metas on those, but the `row`/`col` fields stamped on them are not rewritten — so a coordinate pair is only meaningful for one value of this counter. The async validation flow captures it at the start and drops a result whose coordinates can no longer be trusted. |
...
The map stores materialized values in a single native `Map<number, V>` keyed by the CURRENT volatile key, plus a private logical-length counter that backs the nullish-key `insert`/`remove` ("append"/"remove at end") semantics. A native Map is always a hash table, so far-apart keys (for example, physical row 999990 on a million-row grid) cannot degrade lookups or leave memory residue — do not swap the storage for an array keyed by index, which V8 turns into slow dictionary mode once it goes sparse. The key under which an item was created is volatile — `insert` and `remove` shift it, but lazily: each call only buffers a key-shift operation (contiguous runs — such as `DataMap.removeRow`'s descending per-row calls — merge into one buffered entry, and calls that start at/above every stored key skip buffering entirely). Single-key reads (`obtain`/`has`/`getIfExists`/`evict`) resolve through the buffer by mapping the requested key back to its stored key in O(buffered shifts); only full-view reads (`size`/`values`/`entries`) apply the buffer, in one O(materialized) pass that rebuilds the Map in its original iteration order. So a loop of single-row alters costs one re-key pass per batch, not one per call — do not "fix" the buffering by flushing eagerly. `evict()` is a true `Map.delete` — memory is freed, and `size()` returns the exact materialized count. Iteration (`values()`/`entries()`) yields native lazy iterators in initialization order; deleting the currently visited key mid-iteration (as `evictRow` does) is spec-safe.
