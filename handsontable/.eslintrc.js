...
module.exports = {
  ...
  rules: {
    ...
    'no-restricted-syntax': [
      ...
      // Chrome >= 130, Edge >= 130, Firefox >= 132, Safari >= 18.2). swc lowers *syntax* only — it
      // never injects core-js polyfills — so any instance/static method newer than the oldest
      // targeted engine throws `X is not a function` on a supported browser. The API floor is also
      // pinned as `lib` in ./tsconfig.json (kept in sync with ../browser-targets.js by ES_TARGET),
      // which catches prototype methods this rule would miss; both must be pruned together whenever
      // the floors move. `compat/compat` cannot see these: it does not resolve prototype methods on
      // non-literal receivers, which is how `toSorted` and `Array#at` shipped in 18.0.0.
      //
      // `Array#with` is the only method this repo's own core-js-compat data.json places above the
      // floor: Firefox 140, against our Firefox 132. Its ES2023 siblings toSorted/toSpliced/
      // toReversed sit at Firefox 115, inside the floor, and are allowed.
      // Test files are exempt (no-restricted-syntax is off for them).
      {
        selector: "CallExpression[callee.property.name='with']",
        message: 'Array#with needs Firefox 140+, above the ../browser-targets.js baseline (Firefox >= 132). Use arr.slice() plus an index assignment instead.',
      },
    ],
