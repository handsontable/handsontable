...
export class DateEditor extends TextEditor {
  ...
  refreshDimensions(force: boolean): void {
    ...
    if (
      ...
      const tdOffset = this.TD.getBoundingClientRect();

      this.datePickerStyle.top = `${this.hot.rootWindow.pageYOffset + tdOffset.top + outerHeight(this.TD)}px`;

      let pickerLeftPosition = this.hot.rootWindow.pageXOffset;

      if (this.hot.isRtl()) {
        pickerLeftPosition += tdOffset.right - outerWidth(this.datePicker);
      } else {
        pickerLeftPosition += tdOffset.left;
      }
