 ...
 * @returns {object}
 */
export function useRecorder(ownerWindow: Window, handleEvent: (event: KeyboardEvent) => boolean, beforeKeyDown: (event: KeyboardEvent) => void | boolean, afterKeyDown: (event: KeyboardEvent) => void, callback: (event: KeyboardEvent, keys: string[]) => boolean) {
  /**
   ...
   * @returns {boolean}
   */
  const isModifierKey = (pressedKey: string) => {
   ...
   * @returns {string[]}
   */
  const getPressedModifierKeys = (event: KeyboardEvent, mergeMetaKeys = false) => {
