...
class EventManager {
  ...
  fireEvent(element: Element, eventName: string): void {
    ...
    const options = {
      ...
      metaKey: false,
      button: 0,
      relatedTarget: undefined as unknown as EventTarget,
    };
    let event;
    ...
    if (rootDocument.createEvent) {
      ...
      event = (rootDocument as Document & { createEventObject: () => Event }).createEventObject();
    }

    if (element.dispatchEvent) {
      element.dispatchEvent(event);
    } else {
      (element as unknown as { fireEvent: Function }).fireEvent(`on${eventName}`, event);
    }
  }
}
