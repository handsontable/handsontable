...
export class Search extends BasePlugin {
  ...
  disablePlugin() {
    const beforeRendererCallback = (
      ...
      value: string, cellProperties: CellProperties
  ...
  #onBeforeRenderer = (
    ...
    // array, so the push and splice below never reach a grid-level or column-level array - that is
    // one instance shared by every cell through the cell meta prototype chain.
    const classArray = normalizeClassNames(cellProperties.className);
