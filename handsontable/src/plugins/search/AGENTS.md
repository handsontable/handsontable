...
The result class is applied inside `beforeRenderer`, from the plugin's own state, with no cell meta write. Under `renderMode: 'onChange'` a cell whose value did not change would keep the previous query's class, so `query()` ends with `hot.markAllCellsChanged()`, and so does `disablePlugin()`, whose one-shot `beforeRenderer` strips the class only from the cells the next render paints (a direct `disablePlugin()` + `render()` would otherwise keep the class; `updateSettings({ search: false })` advances the epoch on its own). That hook decides by `this.enabled` (the plugin's runtime state), NOT `isEnabled()` (the `search` setting): a direct `disablePlugin()` leaves the setting `true`, and reading it made the one-shot pass re-add the class it was registered to strip, in every render mode. Do not "optimize" either to the matched cells only: the cells that *stopped* matching need the repaint too.
