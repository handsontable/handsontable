...
const templateContent = ({ title, description, buttons }: { title?: string, description?: string, buttons?: Array<{ type: string, text: string, callback?: Function }> }) => `
...
export class EmptyDataStateUI {
   ...
   */
  updateContent(message: string | Record<string, unknown>) {
    ...
    if (content.buttons?.length && content.buttons.length > 0) {
  ...
  updateSize(view: ViewInstance) {
    ...
    if (rows === 0) {
      if (!this.#placeholderElement) {
        ...
        (view._wt.wtTable.holder as HTMLElement).appendChild(this.#placeholderElement);
      }
    ...
    if (view.isVerticallyScrollableByWindow() || view.hasVerticalScroll()) {
      const hotInstance = (view as ViewInstance & { hot: { getTableHeight(): number } }).hot;

      if (cols > 0) {
        height = hotInstance.getTableHeight() - view.getColumnHeaderHeight();
      } else {
        height = hotInstance.getTableHeight();
      }
