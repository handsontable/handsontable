...
export class DragToScroll extends BasePlugin {
  ...
  enablePlugin() {
    ...
    this.addHook('afterOnCellMouseDown', (event: Event) => this.#setupListening(event as MouseEvent));
    this.addHook('afterOnCellCornerMouseDown', (event: Event) => this.#setupListening(event as MouseEvent));
   ...
   */
  #setupListening(event: MouseEvent) {
    ...
    this.setBoundaries(
      scrollHandler !== this.hot.rootWindow
        ? (scrollHandler as HTMLElement).getBoundingClientRect()
        : undefined
    );

    this.setCallback((scrollX: number, scrollY: number) => {
      const horizontalScrollValue = scrollHandler instanceof HTMLElement
        ? scrollHandler.scrollLeft : (scrollHandler as Window).scrollX;
      const verticalScrollValue = scrollHandler instanceof HTMLElement
        ? scrollHandler.scrollTop : (scrollHandler as Window).scrollY;
