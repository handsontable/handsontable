...
interface HardConflictRegistration {
  blockedKey: string;
  incompatibleSettingKey: string;
}

const hardConflicts: HardConflictRegistration[] = [];
 ...
 */
function isIncompatibleSettingActive(settings: Record<string, unknown>, incompatibleSettingKey: string): boolean {
