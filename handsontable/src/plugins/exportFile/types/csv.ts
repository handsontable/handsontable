...
interface CsvExportOptions {
  ...
  rowDelimiter: string;
  sanitizeValues: boolean | RegExp | ((value: string) => string);
  [key: string]: unknown;
}
...
class Csv extends BaseType {
  ...
  export() {
    ...
    data.forEach((value: unknown, index: number) => {
      ...
      const escapedValue = (value as unknown[])
        .map((cellValue: unknown) => this._escapeCell(cellValue, { sanitizeValue: options.sanitizeValues }))
   ...
   * @returns {string}
   */
  #sanitizeValueWithOWASP(value: string) {
   ...
   * @returns {string}
   */
  #sanitizeValueWithRegExp(value: string, regexp: RegExp) {
