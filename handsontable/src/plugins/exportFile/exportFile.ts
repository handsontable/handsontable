...
export class ExportFile extends BasePlugin {
  ...
  downloadFile(format: string, options: Record<string, unknown> = {}): void {
    ...
    } else if ((navigator as unknown as Record<string, Function>).msSaveOrOpenBlob) { // IE10+
      (navigator as unknown as Record<string, Function>).msSaveOrOpenBlob(blob, name);
    }
  }
  ...
  _createTypeFormatter(format: string, options: Record<string, unknown> = {}): { export: () => string; options: { filename: string; fileExtension: string; [key: string]: unknown } } {
    ...
    return typeFactory(format, new DataProvider(this.hot) as unknown as Record<string, Function>, options) as unknown as { export: () => string; options: { filename: string; fileExtension: string; [key: string]: unknown } };
  }
