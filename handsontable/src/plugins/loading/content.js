 ...
 * @param {function(string): string} options.phraseTranslator Phrase translator function.
 *
 * @returns {string} HTML string with the loading content.
 */
export function loadingContent({ id, icon, title, description, phraseTranslator }) {
  const content = `
    <div class="${LOADING_CLASS_NAME}__content">
      <i class="${LOADING_CLASS_NAME}__icon">${icon}</i>
      <div class="${LOADING_CLASS_NAME}__text">
        <h2 id="${id}-${PLUGIN_KEY}-title" class="${LOADING_CLASS_NAME}__title">
          ${title ?? phraseTranslator(C.LOADING_TITLE)}
        </h2>
        ${description ? `<p 
          id="${id}-${PLUGIN_KEY}-description" 
          class="${LOADING_CLASS_NAME}__description"
        >
          ${description}
        </p>
      ` : ''}
