...
- **`getDataMapAtColumn`/`_readColumn` stamp `visualRow`/`visualCol` with PHYSICAL indexes** (a historical quirk — they pass physical coordinates as the visual options). `stampPhysicalCoordinates()` in `filters.ts` is the one place that writes them. Do not "fix" this by passing true visual indexes, and do not read those stamps expecting visual coordinates.
- **The read resolves no cell meta for a row that stores none until something asks.** A row with stored meta is found with a two-lookup `getCellMetaIfExists` probe during the read; every other row gets `createTransientColumnMeta()` at resolve time, which skips the stored-meta lookup the read already did. Do not reintroduce `getCellMetaUncached()` there — it repeats that lookup. The column-level `valueGetter` probe is hoisted out of the loop, and a row whose value went through one keeps the very object the getter was handed. A guard on "this column stores no meta at all" would be the wrong shape: rendering stores meta for the rows in the eviction band, so on any painted grid it never fires.
...
- **`ConditionUpdateObserver` memoizes the column read, and the memo is what makes the lazy meta safe** per state update / per `flush()` batch. `#withColumnDataCache` opens the scope, `#columnDataCache` holds one `ColumnDataMap` per physical column, and `#getColumnData` reads through it. The cached read is stored as `withMemoizedMeta()`, so a column's meta resolves at most once per row for the whole scope however many passes ask for it — without that, the lazy resolver would rebuild a transient on every pass and hand the same row two different objects. There is **no memo of the materialized entries**: `visibleDataFactory` and `columnValuesFactory` each call `toArray()` and get their own `{row, meta, value}` array (sharing the memoized meta objects), where before this change both were handed the one cached array. That is on the component-update path, not the filter scan. The memo is only sound because source data cannot change inside one update and rows correlate via `entry.row`. If you add a code path that mutates source data during an update cascade, it must not run inside an active memo scope. Subset reads (`physicalRows` argument) intentionally bypass the memo.
