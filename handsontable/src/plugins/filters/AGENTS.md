...
- **A `CONDITION_NONE` state may carry an `itemsSnapshot`.** `ValueComponent.updateState()` stores one whenever the column has conditions but no `by_value` one, and `setState()` restores it for both commands. Every value in that snapshot is checked on purpose: `getState()` then reports `CONDITION_NONE` (via `isSelectedAllValues()`), so opening the menu and confirming it without a change adds no condition. Do not store an unchecked snapshot — it would silently add a `by_value` condition on a no-op interaction.
