...
class MergedCellCoords {
   ...
   * @returns {string}
   */
  static IS_OUT_OF_BOUNDS_WARNING({ row, col }: { row: number, col: number }) {
   ...
   * @returns {string}
   */
  static IS_SINGLE_CELL({ row, col }: { row: number, col: number }) {
   ...
   * @returns {string}
   */
  static ZERO_SPAN_WARNING({ row, col }: { row: number, col: number }) {
   ...
   * @returns {boolean}
   */
  static isSingleCell({ rowspan, colspan }: { rowspan: number, colspan: number }) {
   ...
   * @returns {boolean}
   */
  static containsZeroSpan({ rowspan, colspan }: { rowspan: number, colspan: number }) {
   ...
   */
  normalize(hotInstance: HotInstance) {
   ...
   * @returns {boolean}
   */
  includes(row: number, column: number) {
   ...
   * @returns {boolean}
   */
  includesHorizontally(column: number) {
   ...
   */
  shift(shiftVector: number[], indexOfChange: number) {
   ...
   */
  isFarther(mergedCell: MergedCellCoords | null, direction: string) {
    if (!mergedCell) {
      return true;
    }
