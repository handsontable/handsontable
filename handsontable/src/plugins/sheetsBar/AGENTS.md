...
- **The bar owns the grid-level `dataProvider` gate, not the plugin conflict registry.** `#withBaselineFor()` forces `dataProvider: null` on any sheet whose own `settings` do not declare the key, captures the grid-level value in the settings baseline the first time (so a genuine teardown gives it back via `#restoreBaselineToGrid()`), and warns once (`GRID_LEVEL_DATA_PROVIDER_WARNING`) when that captured value was truthy. `#onAfterUpdateSettings` blocks the same value if it is set later through a plain `updateSettings({ dataProvider })` while the bar is enabled — unless the active sheet's own `settings` declare `dataProvider` itself, which is the user reconfiguring the visible server sheet, not the disabled grid-level value. A `registerConflict('dataProvider', [...])` entry (the mechanism `pagination`, `trimRows` and the rest use) would block *every* sheet's `dataProvider`, including a sheet that declares its own — this needs a per-sheet answer instead, so the bar decides it directly. A sheet's own declared `dataProvider` in its `settings` runs the switch's `updateSettings({ dataProvider })` call like any other overridden key; DataProvider applies it without a refetch (`updatePlugin()` stands aside while `#isSheetSwitching` is true — see `../dataProvider/AGENTS.md`, "SheetsBar: per-sheet server state").
