  ...
  getHeadersSettings(headerLevel, columnIndex, columnsLength = 1) {
    ...
    for (let i = columnIndex; i < headersSettings.length; i++) {
      ...
      if (headerSettings.isPlaceholder) {
        throw new Error('The first column settings cannot overlap the other header layers', { cause: { handsontable: true } });
      }
      ...
      if (currentLength > columnsLength) {
        throw new Error('The last column settings cannot overlap the other header layers', { cause: { handsontable: true } });
      }
    }
