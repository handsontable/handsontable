...
export function normalizeSettings(sourceSettings: unknown[][], columnsLimit = Infinity) {
  ...
  arrayEach(sourceSettings, (headersSettings) => {
    ...
    arrayEach(headersSettings, (sourceHeaderSettings) => {
      const headerSettings = createDefaultHeaderSettings() as NormalizedHeaderSettings;
  ...
  const columnsLength = Math.max(...arrayMap(normalizedSettings, (headersSettings => (headersSettings as unknown[]).length)) as number[]);
