...
- **The one exception is an off-screen SheetsBar sheet.** `#fetchFor` fires `afterDataProviderFetchError` at once, stores the failure in the sheet's state, and resolves `null`. The toast waits until the sheet is visible again (`#onArrival` → `#showDeferredFailure`, which also replays the sheet's last response so its sort, filters, and pager come back). A rejection there would make `crud.ts` revert cells that belong to another sheet. A grid without SheetsBar (`sheetId === null`) is always visible, so it never takes this path.
