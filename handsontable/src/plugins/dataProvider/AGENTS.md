...
A mutation captures `#currentBinding()` when it is queued (`createRows`, `removeRows`, `updateRows`, the `afterChange` path), and `#mutationTarget()` resolves what its follow-up work runs for: a SheetsBar sheet keeps the queued binding, so its callbacks and its refetch serve the sheet it was made on even after a switch; a grid without SheetsBar re-reads the binding when the work runs, exactly as before. An off-screen remove reads the page from the sheet's state and the row count from the sheet's own array, not from the visible grid. A failed `onRowsUpdate` on an off-screen sheet does not run the cell revert (the tuples address the visible grid); `#revertUpdateFor` reloads that sheet from the server instead. Still open: `runUpdateFromChanges` builds its row payloads when the mutation *runs*, from the visible grid, so an edit queued behind a slow mutation and then carried across a switch reads the wrong sheet's rows.
