...
interface CollapsedRowsStash {
  stash(): void;
  applyStash(flag: boolean): void;
  shiftStash(from: number, to: number, count: number): void;
}

interface CollapsingUI {
  areChildrenCollapsed(parent: Record<string, unknown> | null): boolean;
  collapsedRowsStash: CollapsedRowsStash;
}

interface DataManager {
  translateTrimmedRow(rowIndex: number): number;
  isParent(physicalRowIndex: number): boolean;
  isRowHighestLevel(physicalRowIndex: number): boolean;
...
export default class RowMoveController {
   ...
   * @type {NestedRows}
   */
  declare plugin: NestedRowsPlugin;
  /**
   ...
   * @type {CollapsingUI}
   */
  declare collapsingUI: CollapsingUI;
  /**
   * Flag indicating if the rows were moved to a collapsed parent.
   *
   * @type {boolean}
   */
   ...
   */
  displayAPICompatibilityWarning(beforeMoveRowHookArgs: Record<string, unknown>) {
    const {
      rows,
      finalIndex,
      dropIndex,
   ...
   */
  shouldAllowMoving(physicalRowIndex: number, physicalDropIndex: number) {
    /*
   ...
   */
  getBaseParent(physicalStartIndexes: number[]) {
   ...
   */
  getTargetParent(dropToLastRow: boolean, physicalDropIndex: number) {
   ...
   */
  shiftCollapsibleParentsLocations(physicalStartIndexes: number[], physicalDropIndex: number, sameParent: boolean) {
    if (!sameParent) {
   ...
   */
  moveRows(physicalStartIndexes: number[], physicalDropIndex: number, targetParent: Record<string, unknown> | null) {
    ...
    this.hot.batchRender(() => {
      physicalStartIndexes.forEach((physicalStartIndex: number) => {
   ...
   */
  moveCellsMeta(baseIndexes: number[], targetIndex: number) {
    const rowsOfMeta: unknown[] = [];
    const movingDown = Math.max(...baseIndexes) < targetIndex;

    baseIndexes.forEach((baseIndex: number) => {
   ...
   *   [documentation](@/guides/rows/row-moving/row-moving.md).
   */
  selectCells(rows: number[], dropIndex: number | undefined) {
