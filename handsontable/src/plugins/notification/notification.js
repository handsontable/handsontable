...
export class Notification extends BasePlugin {
   ...
   */
  #clearNotificationRegionTabOrder() {
   ...
   * outside the grid.
   */
  #reapplySequentialTabOrderInNotificationHost() {
    const host = this.#ui?.getHost();
   ...
   * @returns {HTMLElement[]}
   */
  #getAllFocusablesInNotificationRegion() {
    const host = this.#ui?.getHost();
   ...
   */
  #getFirstFocusableInNotificationRegion() {
   ...
   */
  #getNotificationRegionFocusableContext() {
    if (this.#toasts.size === 0) {
      return null;
    }
   ...
   * @returns {boolean}
   */
  #shouldHandleTabForwardExitFromNotificationRegion() {
   ...
   * @returns {boolean}
   */
  #shouldHandleShiftTabExitFromNotificationRegion() {
    const ctx = this.#getNotificationRegionFocusableContext();
   ...
   */
  #removeToastState(state, removeDom) {
   ...
   */
  #isActiveInsideAnyRemainingToast(active) {
    let inside = false;
   ...
   */
  #restoreFocusIfIdle(focusInClosingToast = false) {
    const doc = this.hot.rootDocument;
    const host = this.#ui?.getHost();
    const active = doc.activeElement;
   ...
   * @param {string} position Stack key.
   */
  #drainQueueForPosition(position) {
    const limit = this.getSetting('stackLimit');
