describe('WalkontableTable', () => {
  ...
  describe('getFirstVisibleColumn()', () => {
    it('should return -1 error code if there are no rendered rows and columns', async() => {
      ...
      expectWtTable(wt, wtTable => wtTable.getFirstVisibleColumn(), 'master').toBe(-1);

      expect(wt.wtOverlays.bottomInlineStartCornerOverlay).not.toBe(undefined);
      expectWtTable(wt, wtTable => wtTable.getFirstVisibleColumn(), 'bottomInlineStartCorner').toBe(-1);

      expect(wt.wtOverlays.bottomOverlay).not.toBe(undefined);
      expectWtTable(wt, wtTable => wtTable.getFirstVisibleColumn(), 'bottom').toBe(-1);

      expect(wt.wtOverlays.inlineStartOverlay.clone).not.toBe(undefined);
      expectWtTable(wt, wtTable => wtTable.getFirstVisibleColumn(), 'inlineStart').toBe(-1);

      expect(wt.wtOverlays.topInlineStartCornerOverlay).not.toBe(undefined);
      expectWtTable(wt, wtTable => wtTable.getFirstVisibleColumn(), 'topInlineStartCorner').toBe(-1);

      expect(wt.wtOverlays.topOverlay).not.toBe(undefined);
      expectWtTable(wt, wtTable => wtTable.getFirstVisibleColumn(), 'top').toBe(-1);
    });
    ...
    it('should return -1 error code if there are rendered rows and columns after fixed overlays are disabled', async() => {
      ...
      expectWtTable(wt, wtTable => wtTable.getFirstVisibleColumn(), 'master').toBe(0);
