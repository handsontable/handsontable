describe('WalkontableTable', () => {
  ...
  describe('getVisibleColumnsCount()', () => {
    it('should return 0 if there are no rendered rows and columns', () => {
      ...
      expectWtTable(wt, wtTable => wtTable.getVisibleColumnsCount(), 'master').toBe(0);

      expect(wt.wtOverlays.bottomInlineStartCornerOverlay).not.toBe(undefined);
      expectWtTable(wt, wtTable => wtTable.getVisibleColumnsCount(), 'bottomInlineStartCorner').toBe(0);

      expect(wt.wtOverlays.bottomOverlay).not.toBe(undefined);
      expectWtTable(wt, wtTable => wtTable.getVisibleColumnsCount(), 'bottom').toBe(0);

      expect(wt.wtOverlays.inlineStartOverlay.clone).not.toBe(undefined);
      expectWtTable(wt, wtTable => wtTable.getVisibleColumnsCount(), 'inlineStart').toBe(0);

      expect(wt.wtOverlays.topInlineStartCornerOverlay).not.toBe(undefined);
      expectWtTable(wt, wtTable => wtTable.getVisibleColumnsCount(), 'topInlineStartCorner').toBe(0);

      expect(wt.wtOverlays.topOverlay).not.toBe(undefined);
      expectWtTable(wt, wtTable => wtTable.getVisibleColumnsCount(), 'top').toBe(0);
    });
