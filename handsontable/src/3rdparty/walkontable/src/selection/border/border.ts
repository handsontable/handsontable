...
import type { WalkontableInstance, BorderInstanceSettings, CornerDefaultStyle, SelectionHandles } from '../../../../../common';
import {
  addClass,
  hasClass,
  removeClass,
...
class Border {
  ...
  createMultipleSelectorHandles() {
    ...
    const stylesHandler = wtSettings.getSetting('stylesHandler') as import('../../../../../common').StylesHandler;
    ...
    const handleStyle: Record<string, string> = stylesHandler.isClassicTheme() ? {
      position: 'absolute',
      height: `${width}px`,
      width: `${width}px`,
      'border-radius': `${parseInt(String(width / 1.5), 10)}px`,
      background: '#F5F5FF',
   ...
   * @returns {boolean}
   */
  isPartRange(row: number, col: number) {
  ...
  appear(corners: number[]) {
    ...
    const isClassicTheme = (this.wot.wtSettings.getSetting('stylesHandler') as import('../../../../../common').StylesHandler).isClassicTheme();
