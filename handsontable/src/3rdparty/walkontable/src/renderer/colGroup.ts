...
export class ColGroupRenderer extends BaseRenderer {
  ...
  render() {
    ...
    for (let visibleColumnIndex = 0; visibleColumnIndex < rowHeadersCount; visibleColumnIndex++) {
      ...
      (this.rootNode.childNodes[visibleColumnIndex] as HTMLElement).style.width = `${width}px`;
    }
    ...
    for (let visibleColumnIndex = 0; visibleColumnIndex < columnsToRender; visibleColumnIndex++) {
      ...
      (this.rootNode.childNodes[visibleColumnIndex + rowHeadersCount] as HTMLElement).style.width = `${width}px`;
    }
