...
export default class Walkontable extends CoreAbstract {
  ...
  constructor(table: HTMLTableElement, settings: Record<string, unknown>) {
    super(table, new Settings(settings) as unknown as import('../types').WtSettings);
    ...
    this.wtTable = new MasterTable(this.getTableDao() as any, facadeGetter, this.domBindings, this.wtSettings) as unknown as import('../types').WtTable;
    this.wtViewport = new Viewport(
      this.getViewportDao() as any, this.domBindings, this.wtSettings, this.eventManager, this.wtTable
    ) as unknown as import('../types').WtViewport;
    ...
    ) as unknown as import('../types').WtOverlays;
