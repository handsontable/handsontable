...
- **Drop the synthesized sequence symmetrically.** `#isTouchSynthesizedMouseEvent()` gates both `mousedown` and `mouseup`. Dropping only `mouseup` (the original #12804 fix) fired `onCellMouseDown` twice per tap and re-armed the pairing slot and made iPad double-tap-to-edit nondeterministic (DEV-2687). Detection: Blink `sourceCapabilities.firesTouchEvents` via `helpers/dom/inputOrigin.ts` `isTouchSynthesizedMouseEvent()`; WebKit/Firefox fall back to the 500 ms window after the last touch-driven `onMouseUp` (`#lastTouchMouseUpAt`). `tableView.ts` uses the same helper for the outside-click handler.
- **Touch double-tap window is 1000 ms** (`TOUCH_DBLTAP_TIMEOUT`, equal to the mousedown slot). A tap arms both `#dblClickOrigin` slots at `touchend`, so the 500 ms mouseup slot was the binding limit; iOS drops taps under ~250 ms and humans double-tap at 300–600 ms. Mouse double-click timing (1000/500) is unchanged. `TOUCH_SYNTHESIZED_MOUSE_WINDOW` (500 ms) is deliberately shorter than the pairing window, so on the WebKit fallback path a mouse pair arriving 500–1000 ms after a tap completes the double-click; see the constant's JSDoc for the invariants.
