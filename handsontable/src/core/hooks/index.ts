...
export class Hooks {
   ...
   */
  getBucket(context: Record<string, unknown> | null = null): HooksBucket {
    if (context) {
      if (!context.pluginHookBucket) {
        context.pluginHookBucket = new HooksBucket();
      }

      return context.pluginHookBucket as HooksBucket;
    }
   ...
   * ```
   */
  addAsFixed(key: string, callback: HookCallback | HookCallback[], context: Record<string, unknown> | null = null) {
    if (Array.isArray(callback)) {
      arrayEach(callback, (c: unknown) => this.addAsFixed(key, c as HookCallback, context));
    } else {
   ...
   * ```
   */
  remove(key: string, callback: HookCallback, context: Record<string, unknown> | null = null) {
   ...
   */
  has(key: string, context: Record<string, unknown> | null = null) {
   ...
   * ```
   */
  destroy(context: Record<string, unknown> | null = null) {
