...
export class CellRangeToRenderableMapper {
  ...
  toRenderable(range: CellRange) {
    ...
    const to = this.#getNearestNotHiddenCoords(range.to, -rowDirection as 1 | -1, -columnDirection as 1 | -1);
   ...
   */
  #getNearestNotHiddenIndex(indexMapper: IndexMapper, visualIndex: number, searchDirection: 1 | -1) {
    if (visualIndex < 0) {
      return visualIndex;
    }
