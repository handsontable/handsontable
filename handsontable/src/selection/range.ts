...
class SelectionRange {
   ...
   * @returns {SelectionRange}
   */
  push(cellRange: CellRange | unknown) {
    this.ranges.push(cellRange as CellRange);
   ...
   * @returns {boolean}
   */
  includes(coords: CellCoords, criteria: (cellRange: CellRange, index: number) => boolean = () => true) {
    return this.ranges
   ...
   */
  findAll(cellRange: CellRange) {
    const result: { range: CellRange; layer: number }[] = [];
   ...
   * @returns {SelectionRange}
   */
  removeLayers(layerIndexes: number[]) {
   ...
   * @returns {SelectionRange}
   */
  map(mapFunction: (cellRange: CellRange, index: number) => CellRange) {
