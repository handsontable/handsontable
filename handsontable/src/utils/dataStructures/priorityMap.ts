...
const DEFAULT_ERROR_PRIORITY_EXISTS = (priority: unknown) => `The priority '${priority}' is already declared in a map.`;
const DEFAULT_ERROR_PRIORITY_NAN = (priority: unknown) => `The priority '${priority}' is not a number.`;
...
export function createPriorityMap(
   ...
   */
  function addItem(priority: number, item: unknown) {
    if (!isNumeric(priority)) {
      throwWithCause(String(errorNaNFn(priority)));
    }
    if (priorityMap.has(priority)) {
      throwWithCause(String(errorExistsFn(priority)));
    }
