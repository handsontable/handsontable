...
class LinkedList {
   ...
   * @param {object} data Data which should be added.
   */
  unshift(data: Record<string, unknown>) {
    const node = new NodeStructure(data);
  ...
  inorder(callback: Function) {
    ...
    while (temp) {
      const interrupt: unknown = callback(temp);
   ...
   */
  remove(data: Record<string, unknown>) {
    if (this.first === null) {
      return false;
    }
  ...
  recursiveReverse() {
     ...
     */
    function inverse(current: NodeStructure | null, next: NodeStructure | null) {
      if (!next) {
        return;
      }
