...
export default (): Record<string, unknown> => {
  return {
     ...
     * | `initialPage`            | A number (default: `1`)                            | Specifies which page to display on initial load. The plugin applies this value when it first enables and when you change it. Passing the same number again (for example in React state on re-render) does not reset the current page. |
