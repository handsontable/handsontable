...
export default (): Record<string, unknown> => {
  return {
     ...
     * | `splittable`      | `boolean` | Controls what a group does when a column move (with the [`ManualColumnMove`](#manualcolumnmove) plugin) drops a foreign column (one belonging to another group) into its span. When `false` (default), the group adopts that column as a child and stays one banner. When `true`, the group keeps its identity and renders as several same-label banners around the foreign column (it splits). A group always reclaims its own columns when they move back into its span, regardless of this setting. Meaningful only on a header that spans columns. |
     *
     * ::: tip
