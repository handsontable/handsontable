 ...
 */
function getInitiallyCalculatedMoves(movedIndexes: number[], moveLine: number) {
  const moves: Array<{from: number, to: number}> = [];

  movedIndexes.forEach((movedIndex: number) => {
    const move = {
      from: movedIndex,
      to: moveLine,
    };
 ...
 */
function adjustedCalculatedMoves(moves: Array<{from: number, to: number}>) {
  moves.forEach((move: {from: number, to: number}, index: number) => {
    const nextMoved = moves.slice(index + 1);

    nextMoved.forEach((nextMovedIndex: {from: number, to: number}) => {
 ...
 */
export function getMoves(movedIndexes: number[], finalIndex: number, numberOfIndexes: number) {
