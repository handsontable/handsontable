...
- **Measure before claiming a perf win.** A code-level work reduction is a hypothesis until the wall-clock moves — several "obvious" wins here measured null. Bench harness: `handsontable/perf-dev1961-sort-trim-hide.html` (`window.bench` for sort/trim/hide, `window.benchAlter` for the insert/remove index layer) via the chrome-devtools MCP. Note the index layer is a *minority* of an end-to-end `hot.alter()` (the data-array splice + re-render dominate), so optimize against the layer you actually changed.
