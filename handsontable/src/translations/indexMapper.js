...
export class IndexMapper {
  ...
  /**
   * Allows importing the indexes state into the index mapper. The method accepts
   * the object generated by the {@link exportIndexes} method.
   *
   * @param {{
   *   sequence: number[],
   *   trimmingMap: Map<string, number>,
   *   hidingMap: Map<string, number>,
   *   variousMap: Map<string, number>
   * }} indexesState The indexes state to import.
   */
  importIndexes({
    sequence,
    trimmingMap,
    ...
    this.trimmingMapsCollection.importEvery(trimmingMap);
    this.hidingMapsCollection.importEvery(hidingMap);
    this.variousMapsCollection.importEvery(variousMap);
    this.indexesSequence.setValues(sequence);
  ...
  /* eslint-disable jsdoc/require-description-complete-sentence */
  /**
   * Allows exporting the indexes state of the index mapper. The method returns the object
   ...
   */
  exportIndexes() {
    const sequence = this.indexesSequence.getValues();
    const trimmingMap = this.trimmingMapsCollection.exportEvery();
    const hidingMap = this.hidingMapsCollection.exportEvery();
    const variousMap = this.variousMapsCollection.exportEvery();
