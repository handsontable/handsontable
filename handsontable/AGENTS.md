...
This is the core data grid package. **TypeScript** - source files in `src/` are `.ts`. Type definitions in `types/` are auto-generated by `tsc --emitDeclarationOnly` (`npm run build:types`) — do not hand-edit them. Walkontable (`src/3rdparty/`) is excluded from `tsconfig.json` and remains JavaScript.
...
| Walkontable engine | `src/3rdparty/walkontable/src/` (still JS) |
...
| TypeScript definitions (auto-generated) | `types/` |
