...
- **Off-DOM measurement tables (probes/ghosts) must mimic the real grid DOM exactly.** Cell and header styling is scoped to `table.htCore` with child combinators (`table.htCore > thead > tr > th`, `… > tbody > tr > td` — issue #4363), so any table built for `getComputedStyle`/`getBoundingClientRect` measurement needs (1) the `htCore` class on the `<table>` and (2) real `tr` chains — a `<th>` appended straight into `<thead>` matches nothing and measures unstyled. Beware: `.handsontable .htCore` also sets `width: 0; table-layout: fixed`, which breaks content-driven width measurement — neutralize with inline `table-layout: auto; width: auto` (see `utils/ghostTable.ts` `createTable` and `plugins/nestedHeaders/utils/ghostTable.ts`). This bit three times in one PR: the `stylesHandler` box-sizing probe, the core GhostTable header row, and the NestedHeaders ghost table.
