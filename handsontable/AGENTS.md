...
- **Cell `className` is `string | string[]` — never cast it to `string`, and never push into the value you were handed.** The union is declared on `CellMeta` (`src/settings.ts`) and documented with an array example in `metaSchema.ts`; `addClass()` accepts both forms. Normalize with `normalizeClassNames()` from `helpers/dom/element`, which returns a **fresh** array, then write back `join(' ')` — the shape `numericRenderer` and `search` already store. Two traps, both of which have shipped: `(meta.className as string).split(' ')` throws `.split is not a function` on an array, inside a render hook, so the grid renders blank (#7427); and `meta.className += ' marker'` coerces the array through `Array#toString`, silently merging `['a','b']` into one bogus `a,b` class. A third: a grid-level or column-level array is **one instance shared by every cell** through the meta prototype chain, so pushing into it rather than into a copy leaks the marker onto the whole grid (`numericRenderer.ts` still aliases). Match the marker by token, not substring — `indexOf` on the joined string treats a user class named `afterHiddenRowHighlight` as the marker already being present. And in a hook that only sometimes changes the value, compare before assigning: `afterGetCellMeta` runs on every meta read, and an unconditional write materializes an own property that shadows the column-level and grid-level cascade. Type the hook parameter `CellProperties`, not `Record<string, unknown>` — the latter erases the union to `unknown` and is what let the cast compile in the first place.
