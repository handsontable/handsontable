...
- **License key validation is inlined and layered**: typed keys (`[TRIAL]`/`[FREE]`/`[SUB]`/`[PERP]`) are read by `src/utils/typedLicenseKey/` (envelope reader → lifecycle classifier → grants) — a port from the private `handsontable/license-key` repo; never add that package as a dependency. The obfuscated legacy 25-character block in `helpers/mixed.ts` is frozen — typed keys route out BEFORE it via a tag-prefix check, and legacy behavior (including exact console/DOM messages) must never change. The grants API is uniform across key families: legacy/missing/invalid keys resolve to unrestricted grants, so future feature gating can never take a feature away from an existing customer. The badge/popover UI (`src/utils/licenseBranding.ts`) is a core util, NOT a plugin (plugins can be absent or disabled; license UI must not be) and covers non-typed states too (missing/invalid keys get the badge + hover tooltip, non-commercial gets the badge ALONE — no popover, the license permits the usage — and an expired legacy key gets the badge + closable popover; the legacy console/bar messaging stays untouched — `_classifyLegacyKey` in `mixed.ts` mirrors the frozen checks read-only); the corner badge is click-through (`pointer-events: none`) so the corner header keeps its native select-all — hover is detected by delegation (`is-corner-hover` class), and the glyph itself is pure CSS - an `::after` inside the corner header cell (gated by `ht-license-badge-on` on the root element), so it can never overflow or drift out of the corner; never rebuild it as a measured overlay (the overlay version broke visual tests with misaligned badges) - JS measurement exists only to anchor the popover. The hard stops render the CORE-OWNED lock screen (`src/utils/licenseBranding/lockScreen.ts`) — NEVER the Dialog plugin (a shared surface an app legitimately uses: any `show` would replace the lock, any hide would look like a dismissal, and `dialog: true` setups would never tear it down); the trial lock is non-closable, the subscription lock is closable and mode-gated (spec Cases 3a/3b): ONLY payload mode `internal` gets it, `saas`/unknown modes stay console-only — no lock, no bar, no badge. The whole branding surface remounts on a runtime `licenseKey` change (memoized on the key string — resolution checksums the key), so fixing a key via `updateSettings` releases locks and removes badges without a rebuild. Test fixtures: real CLI-generated keys in `src/utils/typedLicenseKey/__tests__/fixtures.js` plus a test-only key builder (`buildTestKey.js`) that must never be imported from `src/`.
