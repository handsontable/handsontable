...
- **Wrapper UI placement**: The root wrapper has slots `ht-before-grid`, `ht-grid` (grid + empty-data-state), `ht-after-grid`, `ht-overlays` (last). Add plugin UI to a user-orderable edge slot via `hot.getLayoutManager().register(key, element, { side: 'before'|'after', weight })` and `unregister(key, side)` on teardown — `register` owns the DOM placement, so do NOT `appendChild` into the slot elements yourself. The `overlays` slot is internal (not user-orderable through the `layout` setting); reach it with the lower-level `getLayoutManager().getSlot('overlays').add/remove` (which `register`/`unregister` wrap for the edge slots). `getLayoutManager()` throws on non-root instances, so guard slot calls with `isRootInstance(this.hot)`. Edge-slot order is user-overridable through the `layout` setting (`beforeGrid`/`afterGrid` only). `ht-grid` is internal and not orderable. `LayoutManager.destroy()` keeps its slot map (only clears contents) so a plugin's `disablePlugin` running after core destroy stays safe. See `src/core/layout/`.
