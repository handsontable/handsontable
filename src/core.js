...
Handsontable.Core = function (rootElement, settings) {
  ...
  datamap = {
    ...
    /**
     * Returns the next column to jump for Ctrl+Arrow and Ctrl+Shift+Arrow
     * @param {integer} 1 for right, -1 for left
     * @returns {integer}
     */
    getNextColToJump: function (curRow, curCol, dir) {
    	var range;
    	if(dir == 1){
    		 ...
    		 for (var r = 0; r < range.length; r++) {
 		        for (var c = range[r].length-2; c >=0 ;c--) {
 		        	if(range[r][c] && range[r][c] != ''){
 						...
 						return curCol - (range[r].length-1 - c-1);
					}else {
						isNonContinuous = true;
					}
 		        }
    ...
    /**
     * Returns the next row number to jump for Ctrl+Arrow and Ctrl+Shift+Arrow
     * @param {integer} 1 for down, -1 for up
     * @returns {integer}
     */
    getNextRowToJump: function (curRow, curCol, dir) {
    	var range;
    	if(dir == 1){
    		...
    		var isNonContinuous = (range[range.length -1][0] && range[range.length -1][0]) != '' ? false:true;
    		 for (var r = range.length -2; r >=0 ; r--) {
 		        	if(range[r][0] && range[r][0] != ''){
 						...
 						return curRow - (range.length-1 - r -1);
 					}else {
						isNonContinuous = true;
					}
 		      }
          ...
          switch (event.keyCode) {
            case 38: /* arrow up */
              if(event.ctrlKey && event.shiftKey){
            	  var nextFilledRow = datamap.getNextRowToJump(priv.selEnd.row(), priv.selStart.col(), -1);
          		  selection.transformEnd(nextFilledRow -priv.selStart.row() - ( priv.selEnd.row() - priv.selStart.row()), 0);
          	  }
          	  else if (event.shiftKey) {
                selection.transformEnd(-1, 0);
              }else if(event.ctrlKey){
            	  rangeModifier({row: datamap.getNextRowToJump(priv.selStart.row(), priv.selStart.col(), -1), col: priv.selStart.col()});
              }
              else {
            ...
            case 39: /* arrow right */
        	  if(event.ctrlKey && event.shiftKey){
        		 var nextFilledColumn = datamap.getNextColToJump(priv.selStart.row(), priv.selEnd.col(), 1);
        		 //console.log(priv.selStart.row() +" "+priv.selStart.col() + priv.selEnd.row() +" "+priv.selEnd.col() + " " + nextFilledColumn);
        		 selection.transformEnd(0, (nextFilledColumn - priv.selStart.col() - ( priv.selEnd.col() - priv.selStart.col())));
        	  }
        	  else if (event.shiftKey) {
                selection.transformEnd(0, 1);
              }else if(event.ctrlKey){
            	  rangeModifier({row: priv.selStart.row(), col:  datamap.getNextColToJump(priv.selStart.row(), priv.selStart.col(), 1)});
              }
              else {
