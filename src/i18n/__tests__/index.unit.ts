...
import { register as registerPhraseFormatter } from '../phraseFormatters';
import plPL from '../languages/pl-PL';
import * as allLanguages from '../languages';
...
describe('i18n', () => {
  ...
  describe('Collection of language packs', () => {
    ...
    const languagePacks = filesInsideLanguageDictionary.filter(name => !name.includes('index'));
      ...
      describe('File `src/i18n/languages/index.js`', () => {
        ...
        it('should provide export names corresponding to dictionaries file names', () => {
          ...
          expect(indexFileExportNames).toBeListFulfillingCondition(exportName =>
            languagePacks.map(languagePack => languagePack.replace('.ts', '').replace('-', '')).includes(exportName));
        });
      });
      ...
      describe('Particular dictionary', () => {
        ...
        it('should have file name corresponding to its language code', () => {
          ...
          expect(languagePacks)
            .toBeListFulfillingCondition(languagePack => languageCodes.includes(languagePack.replace('.ts', '')));
        });
