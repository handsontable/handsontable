...
export default class ViewDiffer {
  ...
  appendSize(sizeSet) {
    this.sizeSet.append(sizeSet.size);
  }

  prependSize(sizeSet) {
    this.sizeSet.prepend(sizeSet.size);
  }
  ...
  diff() {
    const {
      currentOffset, currentViewSize,
      nextOffset, nextViewSize
    } = this.sizeSet.getSize();

    // @TODO(perf-tip): Creating an array (createRange) is not necessary it would be enought to generate commands based on numeric values.
    ...
    for (let i = 0; i < maxSize; i++) {
      ...
      // Current index exceeds the next DOM index so it is necessary to generate a "remove" command to achieve new order.
      if (nextIndex === void 0) {
        leads.push(['remove', currentIndex]);

      // Next index exceeds the current DOM index so it is necessary to generate a "append" command to achieve new order.
      } else if (currentIndex === void 0) {
        // Check what command should be generated (depends on if this work as a shared root node or not)
        if (this.sizeSet.canAppend(nextIndex)) {
          /**
          ...
          /**
           * If the differ is sharing root node "prepend" command is generated.
           *
           * For example:
           *     ┌─────────── <tr> (root node, shared between two differs)
           *  (first differ)    <th> <--- Generates "insert_shared" command (which later executes `rootNode.insertBefore(node, rootNode.firstChild)`)
           *     ├───────────   <td>
           *     │              <td>
           *  (second differ    <td>
           *   - TDs)           <td>
