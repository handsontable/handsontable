  ...
  describe('API', () => {
    describe('getDataObject', () => {
      it('should return the data source object corresponding to the provided visual row number', () => {
        ...
        expect(plugin.dataManager.getDataObject(0)).toEqual(sourceDataReference[0]);
        expect(plugin.dataManager.getDataObject(2)).toEqual(sourceDataReference[0].__children[1]);
        expect(plugin.dataManager.getDataObject(4)).toEqual(sourceDataReference[0].__children[2].__children[0]);
        expect(plugin.dataManager.getDataObject(6)).toEqual(sourceDataReference[0].__children[3]);
        expect(plugin.dataManager.getDataObject(11)).toEqual(sourceDataReference[2].__children[1].__children[0]);
    ...
    describe('getRowIndex', () => {
      it('should return a visual row index for the provided source data row object', () => {
        ...
        expect(plugin.dataManager.getRowIndex(sourceDataReference[0])).toEqual(0);
        expect(plugin.dataManager.getRowIndex(sourceDataReference[0].__children[2])).toEqual(3);
        expect(plugin.dataManager.getRowIndex(sourceDataReference[0].__children[2].__children[0])).toEqual(4);
        expect(plugin.dataManager.getRowIndex(sourceDataReference[0].__children[3])).toEqual(6);
        expect(plugin.dataManager.getRowIndex(sourceDataReference[2].__children[1].__children[0])).toEqual(11);
      });
    });
    ...
    describe('countChildren', () => {
      ...
      it('should return a number of children (and children\'s children) of the row provided as a row object from the data source', () => {
        ...
        expect(plugin.dataManager.countChildren(sourceDataReference[0])).toEqual(6);
        expect(plugin.dataManager.countChildren(sourceDataReference[0].__children[2])).toEqual(2);
        expect(plugin.dataManager.countChildren(sourceDataReference[0].__children[2].__children[0])).toEqual(1);
        expect(plugin.dataManager.countChildren(sourceDataReference[0].__children[3])).toEqual(0);
        expect(plugin.dataManager.countChildren(sourceDataReference[2].__children[1].__children[0])).toEqual(0);
    ...
    describe('getRowParent', () => {
      it('should return a row object from the data source, being the parent node for the provided row index', () => {
        ...
        expect(plugin.dataManager.getRowParent(3)).toEqual(sourceDataReference[0]);
        expect(plugin.dataManager.getRowParent(4)).toEqual(sourceDataReference[0].__children[2]);
        expect(plugin.dataManager.getRowParent(6)).toEqual(sourceDataReference[0]);
        expect(plugin.dataManager.getRowParent(11)).toEqual(sourceDataReference[2].__children[1]);
      ...
      it('should return a row object from the data source, being the parent node for the provided row object', () => {
        ...
        expect(plugin.dataManager.getRowParent(sourceDataReference[0])).toEqual(null);
        expect(plugin.dataManager.getRowParent(sourceDataReference[0].__children[2])).toEqual(sourceDataReference[0]);
        expect(plugin.dataManager.getRowParent(sourceDataReference[0].__children[2].__children[0])).toEqual(sourceDataReference[0].__children[2]);
        expect(plugin.dataManager.getRowParent(sourceDataReference[0].__children[3])).toEqual(sourceDataReference[0]);
        expect(plugin.dataManager.getRowParent(sourceDataReference[2].__children[1].__children[0])).toEqual(sourceDataReference[2].__children[1]);
    ...
    describe('getRowLevel', () => {
      ...
      it('should return a row object from the data source, being the parent node for the provided row object', () => {
        ...
        expect(plugin.dataManager.getRowLevel(sourceDataReference[0])).toEqual(0);
        expect(plugin.dataManager.getRowLevel(sourceDataReference[0].__children[2])).toEqual(1);
        expect(plugin.dataManager.getRowLevel(sourceDataReference[0].__children[2].__children[0])).toEqual(2);
        expect(plugin.dataManager.getRowLevel(sourceDataReference[0].__children[3])).toEqual(1);
        expect(plugin.dataManager.getRowLevel(sourceDataReference[2].__children[1].__children[0])).toEqual(2);
    ...
    describe('addChild', () => {
      it('should add an empty child to the provided parent, when the second method arguments is not declared', () => {
        ...
        const plugin = hot.getPlugin('nestedRows');
        const parentElement = sourceDataReference[0].__children[2];
    ...
    describe('detachFromParent', () => {
      it('should detach a child node from it\'s parent and re-attach it to the parent of it\'s parent', () => {
        ...
        const plugin = hot.getPlugin('nestedRows');
        let parentElement = sourceDataReference[0].__children[2];
        let grandparent = plugin.dataManager.getRowParent(parentElement) || sourceDataReference;
        let child = parentElement.__children[0];

        expect(parentElement.__children.length).toEqual(1);
        expect(grandparent.__children ? grandparent.__children.length : sourceDataReference.length).toEqual(4);

        plugin.dataManager.detachFromParent(child);

        expect(parentElement.__children.length).toEqual(0);
        expect(grandparent.__children ? grandparent.__children.length : sourceDataReference.length).toEqual(5);
        expect(grandparent.__children ? grandparent.__children[4] : sourceDataReference[4]).toEqual(child);

        parentElement = sourceDataReference[2];
        grandparent = plugin.dataManager.getRowParent(parentElement) || sourceDataReference;
        child = parentElement.__children[1];

        expect(parentElement.__children.length).toEqual(2);
        expect(grandparent.__children ? grandparent.__children.length : sourceDataReference.length).toEqual(3);

        plugin.dataManager.detachFromParent(child);

        expect(parentElement.__children.length).toEqual(1);
        expect(grandparent.__children ? grandparent.__children.length : sourceDataReference.length).toEqual(4);
        expect(grandparent.__children ? grandparent.__children[3] : sourceDataReference[3]).toEqual(child);
      });
    });
  });
});
