...
function Autofill(instance) {
  ...
  eventManager.addEventListener(document,'mousemove', function (event){
    ...
    var tableBottom = dom.offset(plugin.instance.table).top - (window.pageYOffset || document.documentElement.scrollTop) + dom.outerHeight(plugin.instance.table)
      , tableRight = dom.offset(plugin.instance.table).left - (window.pageXOffset || document.documentElement.scrollLeft) + dom.outerWidth(plugin.instance.table);
