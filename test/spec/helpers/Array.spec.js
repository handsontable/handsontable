describe('Array helper', function() {
  ...
  // Handsontable.helper.arrayIncludes
  //
  describe('arraySortByMergeSort', function() {
    it('should sort properly array containg strings', function() {
      var arraySortByMergeSort = Handsontable.helper.arraySortByMergeSort;

      expect(arraySortByMergeSort(['z', 'c', '', '1', 'a'])).toEqual(['', '1', 'a', 'c', 'z']);
    });

    it('should sort lexically array containg numbers (by default)', function() {
      var arraySortByMergeSort = Handsontable.helper.arraySortByMergeSort;

      expect(arraySortByMergeSort([3, 40, 100, 2000])).toEqual([100, 2000, 3, 40]);
    });

    it('should sort array containg numbers (when comparing function is passed)', function() {
      var arraySortByMergeSort = Handsontable.helper.arraySortByMergeSort;
      var compareFunction = function (leftValue, rightValue) {
      ...
      expect(arraySortByMergeSort([1, 2, 11, 3], compareFunction)).toEqual([1, 2, 3, 11]);
    });

    xit('should handle NaN properly when sorting with primitive values', function() {
      var arraySortByMergeSort = Handsontable.helper.arraySortByMergeSort;

      expect(arraySortByMergeSort([NaN, 1, 0, NaN, NaN, 0, NaN, 3, NaN, NaN, 0, NaN]))
        .toEqual(
          [0, 0, 0, 1, 3, NaN, NaN, NaN, NaN, NaN, NaN, NaN]
        );

      expect(arraySortByMergeSort([NaN, 1, 0, NaN, NaN, 0, NaN, 3, NaN, NaN, 0, NaN]))
        .toEqual(
          [NaN, 1, 0, NaN, NaN, 0, NaN, 3, NaN, NaN, 0, NaN].sort()
        );
    });
  });
});
