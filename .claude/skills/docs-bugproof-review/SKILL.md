...
| 8 | If the diff touches anything outside `docs/content/**` or `docs/public/img/**` (e.g. `docs/cloudflare/_worker.js`, `docs/astro.config.mjs`, `docs/public/scripts/*.js`), say so explicitly in the PR description: `docs-sync.yml` only auto-ports content-only commits to `prod-docs/<major>.<minor>`, so this kind of change needs a manual port or it silently never reaches the live site. | A feature that works on `develop` but is invisible in production, with nothing flagging why | #13516 (new Figma-date feature spanned `_worker.js`/`astro.config.mjs`/a public script -- reviewer had to point out none of that reaches prod through the normal sync) |
| 9 | When you move content from one file to another (a refactor, a slimming-down, a split), grep the *old* file's surroundings and the *new* file's incoming text for relative pointers -- "the rule above", "see below", "as noted earlier" -- that assumed same-file adjacency. Moving the content across files breaks these; name the file explicitly instead. | Dangling "above"/"below" references after a cross-file move | #13507 (two rules moved from `AGENTS.md` into `.ai/CI.md` kept saying "the rule above", which no longer had anything to point at from a different file) |
...
- "It builds/renders on develop, so it's live." -- develop is not production; anything outside `docs/content/**`/`docs/public/img/**` needs a manual port to `prod-docs/<major>.<minor>` or it never ships. (#13516)
- "I moved the section, the words are still accurate." -- accurate words that say "above"/"below" can still point at nothing once they're in a different file. (#13507)
...
5. For item 8, if any changed file falls outside `docs/content/**`/`docs/public/img/**`, say in the PR description that it needs a manual prod-docs port.
6. For item 9, if the diff moves or deletes prose from one file into another, grep both sides for "above"/"below"/"earlier"/"this section" and fix any that no longer resolve.
7. Only after all nine pass, tell the user the PR is ready for human review -- and say plainly which items you actually verified (e.g. "checked JS+source for the dictionary-registration behavior; ran the demo for the hook-firing claim") rather than a bare "looks good."
