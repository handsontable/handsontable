...
- **Strip the index signature before `Omit`/`Pick` on a settings type.** `HotTableProps`/`HotColumnProps` derive from core `GridSettings`/`ColumnSettings`, which carry a `[key: string]: any` escape hatch for plugin/meta keys. Wrap the input in `RemoveIndexSignature<T>` first (as `ReplaceRenderersEditors` does) so the named options survive `Omit` — without it `keyof` widens to `string`, every option name is dropped, and IDE autocomplete inside `<HotTable>`/`<HotColumn>` breaks. (Interface `extends` keeps the options but can't override `renderer`/`editor`/`data`, whose types differ.)
