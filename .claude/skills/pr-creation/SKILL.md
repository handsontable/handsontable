...
The changelog gate is path-aware: a PR confined to docs, tests, `.github/`, `.ai/`, `.claude/`, `visual-tests/`, or `tests/` passes it with no entry and needs no `[skip changelog]` — do not write the marker there. Write `[skip changelog]` in the PR body only to deliberately skip the entry on a genuine change under `handsontable/src/**` or `wrappers/**`, and say why in the Context. When no entry is needed there is no PR-first round-trip — open the PR and be done. A PR that grows the visual golden set declares it with `[visual budget: N – reason]` instead (the template's comment block explains N); that marker is unrelated to the changelog.
