...
| `handsontable/styles/` | Compiled CSS output — **gitignored** (`handsontable/.gitignore`), NOT committed; `git ls-files handsontable/styles/` returns nothing. Regenerated by `build:styles`/`build:themes-css`. |
...
Every built-in icon is a real element, not a CSS pseudo-element: `<i class="ht-icon ht-icon-<kebab-name>" aria-hidden="true"></i>`. Two functions build it, both in `handsontable/src/themes/engine/icons.ts`:

- `createIcon(hot, name, options)` — builds a fresh icon element. `options` is `{ flipInRtl?, className? }`.
- `syncIcon(hot, container, slotClass, name, options)` — keeps exactly one icon in a slot of a container a hook re-renders: creates it when missing, leaves it alone when the name is unchanged, replaces it when the name changed, and **removes it when `name` is `null`**. Use this (not `createIcon` + manual bookkeeping) anywhere a header/draw hook decides per render whether an icon shows at all.

The glyph itself comes from `--ht-icon-<name>` (a CSS custom property emitted per theme) plus a generated `.ht-icon-<name>` rule (`mask-image: var(--ht-icon-<name>); background-color: currentColor`). `handsontable/src/styles/components/core/_icon.scss` defines `.ht-icon` — **it carries NO paint**: no `background-color`, no `mask-image`. That is load-bearing, not an oversight: a `-no-icons` bundle (`ht-theme-<name>-no-icons.css`) ships neither the CSS variable nor the `.ht-icon-<name>` rule, so an undefined `--ht-icon-*` resolves `mask-image` to `none`. If the base class painted `background-color: currentColor` itself, every icon in a `-no-icons` bundle would render as a solid square instead of nothing. Never add paint to `.ht-icon` — only to a generated `.ht-icon-<name>` rule.

### Glyph vs. chrome — don't retire the wrong pseudo-element

Several migrated components had TWO pseudo-elements on the same host: one drawing the glyph, one drawing a background/border/ring/hover surface behind or around it. Only the glyph one is replaced by the real `<i>` element and has its `@include mixins.pseudo` (`content: ''`) dropped. The chrome pseudo-element keeps its `content` and stays exactly as it was. Mixing the two up shipped wrong four separate times in this migration: the radio button's focus ring, the sheets-bar chevron's hover surface, the checkbox's box, and the collapsible-column indicator's background layer. Before touching a component's `&::before`/`&::after` pair, work out which one is the icon and which one is decoration underneath it — read both rules, don't assume `::before` is always the glyph.

### RTL mirroring: never let positioning set `transform` on a flipped icon

An icon that must mirror under RTL carries the `ht-icon--flip-rtl` modifier class, whose rule is `[dir="rtl"] .ht-icon--flip-rtl { transform: scaleX(-1); }`. **That icon's own positioning rule must never also set `transform`** (e.g. `transform: translateY(-50%)` for vertical centering) — the two declarations collide on the same property, the later/more-specific one wins, and the mirror silently stops working. This is dangerous precisely because it looks fine: the icon still renders in the right place in LTR, and the bug only shows up under `[dir="rtl"]`. Centre such an icon with a negative margin or by putting it inside a flex/grid container instead of `transform: translate(...)`.

### Migrating a site: move the STATE rules too, not just the glyph

When retiring a pseudo-element icon in favor of `createIcon`/`syncIcon`, any `:hover`, `:focus`, `:disabled`, `:checked` or `.is-active`-style rule that colored the OLD pseudo-element must be re-keyed onto `.ht-icon` (or a class the real `<i>` element carries). Forgetting this leaves the chrome around the icon responding to state changes while the glyph itself stays visually dead.

### Adding or renaming an icon touches four lists, not one

An icon name lives in four places that a test or the type checker cross-checks — see `handsontable/scripts/themes/figma/.ai/CONVENTIONS.md` ("Adding or changing an icon") for the generator side and `IconKey`/`ICON_NAMES`/`VALID_ICON_KEYS` for the runtime side. Missing one of the four fails a unit test or `tsc`, not silently.
