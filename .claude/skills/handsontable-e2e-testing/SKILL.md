...
Use `const layout = getThemeLayout()` (token-backed; merged API from `test/helpers/themeLayoutFromTokens.js`, which composes `themeLayoutCore.js` + `themeLayoutE2eHelpers.js`). Prefer primitives (`layout.defaultDataRowHeight`, `layout.overlayHeight()`, …), formulas (for example `layout.defaultDataRowHeight + 29`, `Math.max(30, layout.firstRenderedRowDefaultHeight)`), and named **`layout.e2e*()`** / **`layout.e2eGcr_*()`** helpers for scenario-specific triplets. For one-off literals that still differ by density (classic → **`compact`**, main → **`default`**, horizon → **`comfortable`**), use **`layout.e2ePickForDensity({ compact, default, comfortable })`** in the spec instead of three separate tests. Add new **`e2e*`** helpers in `themeLayoutE2eHelpers.js` (they may call **`pickByDensity` internally** with `{ compact, default, comfortable }`). Add new themes and token primitives in `themeLayoutCore.js`. **Do not** call **`pickByDensity`** from **`*.spec.js`** files (ESLint `handsontable/no-pick-by-density-in-spec`). Add **targeted** unit tests in `themeLayoutFromTokens.unit.js` for token-derived behavior, not bulk loops that only mirror helper outputs. For inner Handsontable editor lists (dropdown / handsontable / autocomplete), use global **`expectInnerHandsontableEditorListClientBoxMatchesSettings()`** from `test/helpers/common.js`. For **`getEditedCellRect`**, use **`expectGetEditedCellRectFromPartial`** with **`layout.e2eGcr_*`**; pass **`getE2eDocumentViewport()`** (or `const v = getE2eDocumentViewport(); … v.clientWidth, v.clientHeight, v`) when the helper needs document scroll or viewport height. For dropdown placement vs the cell, use **`getBoundingClientRect()`** vs `editor.TD` where possible instead of absolute document offsets.

**Do not** branch on `getLoadedTheme()` in spec files for pixel expectations. Helpers use **`layout.densityLevel`** (`compact` / `default` / `comfortable`) inside `themeLayoutE2eHelpers.js`. Adding a theme registers `THEME_TOKENS` + `THEME_DENSITY` in `themeLayoutCore.js` only; extend `e2e*` methods in `themeLayoutE2eHelpers.js` if a new density needs different regression math. For mixed matchers (for example `toBeGreaterThan` on one density and `toEqual` on others), use **`layout.densityLevel`** in the spec or a small local helper.
