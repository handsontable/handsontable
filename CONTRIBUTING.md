...
**The tracked human exception.** When automated coverage genuinely cannot judge a change (a subtle UX or visual nuance, a high-risk area), tick **"MANUAL QA NEEDED"** in the PR description (the template carries the line) and say in one line what to check. The PR then gets the red **`Requires Manual QA`** label, and the Tests pipeline holds its **`Manual QA / sign-off`** job until a designated reviewer approves the run (the **Review pending deployments** button on the workflow run) — the approver is recorded by GitHub, and self-approval is blocked. Unticked PRs skip the job. Ticking the box after the pipeline already ran arms the gate anyway — the Tests run is re-triggered automatically — so a green PR cannot stay green once you ask for a manual pass. Unticking reverses nothing on its own: take the label off by hand, and press **"Re-run all jobs"** to drop a gate that is already waiting. This *adds* a recorded human pass — it never replaces the test requirement.
