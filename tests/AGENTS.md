...
`.github/scripts/__tests__/playwright-flake-settings.test.mjs` pins both (and
`forbidOnly`) as text in the root `test:tooling` gate, and fails with this
reason if one moves. The report path is pinned by
`.github/scripts/lib/test-health.mjs` and asserted in
