...
`networkidle`, `.only`, `.skip`, or bare `test.fixme` in specs, and no
`waitForFunction()` without an explicit `{ polling }` — the rAF default is
...
type assertion, is judged; a plain options variable is not). Wait on
