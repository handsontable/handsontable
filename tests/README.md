...
> Current state: this package pins `1.61.1` concretely and is verified against it. Moving it (and the others) onto `catalog:` and adding `tests/` to the workspace regenerates `pnpm-lock.yaml`, so it is done as one reviewed change.
