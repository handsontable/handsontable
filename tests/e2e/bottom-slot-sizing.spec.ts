...
for (const plugin of PLUGINS) {
  test.describe(`${plugin} bar in a grid without a height option`, () => {
    ...
    test('keeps the bar directly below the clipped table with an explicit height (control)', async () => {
      ...
      expect(geometry.root.height).toBeLessThanOrEqual(400);
    });
  });
}
...
test.describe('regression guards around the wrapper and slot sizing', () => {
  ...
  // The wrapper-height change must leave the plain window-scroll layout alone: the rows still
  // belong to the page and stay virtualized. The license notification is the only slot content.
  ...
  // wrapper must stay pinned to the container.
  test('keeps a `height: "100%"` grid filling its CSS-sized container', async () => {
    ...
    expect(geometry.verticalByWindow).toBe(false);
    expect(geometry.wrapperClasses).not.toContain('ht-vertical-window-scroll');
  ...
  // The wrapper follows the table's content in window mode, so a data change now resizes the
  // element the engine's ResizeObserver watches. The loop guard must keep that quiet.
  test('grows with inserted rows without a resize loop or console error', async ({ page }) => {
    const errors: string[] = [];
    ...
    expect(after.wrapper.height).toBeGreaterThan(before.wrapper.height);
