...
CopyPasteClass.prototype.init = function () {
  ...
  if (document.getElementById('CopyPasteDiv')) {
    ...
    this.elTextarea.onpaste = function(event) {
      var clipboardContents, clipboardHtml,
        temp;
      ...
      if (event.clipboardData && event.clipboardData.getData){
        ...
        //if('MozAppearance' in window.document.documentElement.style) {
          //clipboardHtml = event.clipboardData.getData("text/plain");
        //}        
      } else if(window.clipboardData && window.clipboardData.getData) {  // support IE
...
CopyPasteClass.prototype.copyable = function(string) {
  ...
  //将需要copy的内容分为两部分： 表格内粘贴所需的html，表格外粘贴的text
  var parsedStr = SheetClip.parse(string),
    rowItem, rowSet, dataSet=[];
  for(var row=0,l=parsedStr.length; row<l; row++){
    rowSet = [];
    for(var col=0,len=parsedStr[row].length; col<len; col++){
      rowItem = $(parsedStr[row][col]).text();
      rowSet.push(rowItem);
    }
    dataSet.push(rowSet);
  }
  // if(!SheetClip.stringify(dataSet).trim()){
  //   return;
  // }
...
CopyPasteClass.prototype.triggerPaste = function(event, string) {
  ...
  function _escapeStr(str) {
    ...
    if (str.substr(0, 1) === '"' && str.substr(-1) === '"') {
      escapedStr = '"' + _.escape(str.substr(1, str.length-2)) + '"';
    } else {
