...
import { fenceForSummary, scrubSecrets, stripTaskIds } from '../lib/docs-sync/log.mjs';
...
test('stripTaskIds removes ClickUp ids, keeps CVE ids whole, and preserves newlines', () => {
  // Leading prefix, mid-text, and a short 1-digit id.
  assert.equal(stripTaskIds('DEV-2894: Americanize spelling'), ': Americanize spelling');
  assert.equal(stripTaskIds('Fix the SU-833 regression'), 'Fix the regression');
  // No trim: a lone leading space is left for the caller (plain() trims, the
  // commit-message path cleans a leading `: `).
  assert.equal(stripTaskIds('IT-4 gateway note'), ' gateway note');

  // A CVE id is left whole by the negative lookahead.
  assert.equal(stripTaskIds('Bump x to fix CVE-2024-12345'), 'Bump x to fix CVE-2024-12345');

  // Newlines survive, so a multi-line commit message keeps its structure and
  // its cherry-pick trailer.
  const message = 'DEV-1: Fix\n\nbody line\n\n(cherry picked from commit abcdef1234567)';

  assert.equal(
    stripTaskIds(message),
    ': Fix\n\nbody line\n\n(cherry picked from commit abcdef1234567)',
  );
});
