...
- [ ] This change needs a manual QA pass — apply the `needs-manual-check` label and give the reason here: <!-- one line: WHAT to check and why automation can't judge it. The Tests run then waits for a manual-qa environment approval by a designated reviewer (never the PR author); label added after the run finished? press "Re-run all jobs" -->
