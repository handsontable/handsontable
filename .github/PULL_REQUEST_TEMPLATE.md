...
- [ ] This change needs a manual QA pass — <!-- one line: WHAT to check and why automation can't judge it. Ticking this makes the Tests run wait for a manual-qa environment approval by a designated reviewer (never whoever triggered the run); ticking it later re-runs the Tests pipeline automatically to arm the gate, and applies the red `Requires Manual QA` label. Unticking does NOT lift a gate already waiting — press "Re-run all jobs" for that. This line is machine-read — keep its wording. -->
