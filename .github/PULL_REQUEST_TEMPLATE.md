...
- [ ] MANUAL QA NEEDED — <!-- one line: WHAT to check and why automation can't judge it. Ticking this holds the Tests run for a manual-qa environment approval by a designated reviewer (never whoever triggered the run) and adds the red `Requires Manual QA` label; tick it later and the pipeline re-runs itself to arm the gate. Unticking removes nothing — take the label off by hand, and press "Re-run all jobs" to drop a gate already waiting. This line is machine-read — keep its wording. -->
