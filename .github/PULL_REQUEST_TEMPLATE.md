...
- [ ] Manual QA Needed
<!-- Tick "Manual QA Needed" only when automation cannot judge the change (a subtle UX or visual nuance, a high-risk area), and then ALSO apply the red `Requires Manual QA` label — the Checks pipeline fails a ticked PR that lacks it (agents: `gh pr create --label "Requires Manual QA"`). Say in the Context section what to check. Ticking holds the Tests run for a manual-qa environment approval by a designated reviewer (never whoever triggered the run); the box is read once per run, so if you change it after the pipeline ran, press "Re-run all jobs". The checklist line above is machine-read — keep its wording exactly. -->
